...
**`tos/JCTsh-Session-Start.md`** — the ordered checklist (git status, the kanban board's
Build column and recent activity, open PRs, Auto verify/Watch for markers, the process doc,
archiving, and the Log Dashboard scan), plus the repo's timestamp-format rule. Read every
session, not just the first time — same standing as `tos/JCTsh-Operating-System.md` below.
That file also covers which steps a component/cluster session (CARD-0284) runs differently.
...
```
Find entry IDs via `GET /api/config/config_entries/entry`, filtered by `domain` (whichever integration's entities look unavailable, or whichever appeared in the boot log's slow-loading list — not a fixed list of domains to check). Confirmed 2026-09-05: after a routine HA update, 150 of 203 unavailable entities were Ring/SmartThings entities that needed exactly this reload — the config entry itself reported `state: loaded` the whole time, so `loaded` doesn't guarantee actually synced.
...
- `hosts/pi1/daemon.json` — the **Pi's** Docker engine config (DNS pinning + `data-root` on the USB drive, CARD-0159). Deploy: stage to `/tmp`, `sudo install` to `/etc/docker/daemon.json`, `sudo systemctl restart docker`.
- `hosts/m8/daemon.json` — the **M8's** Docker engine config (DNS pinning + `log-driver: journald`, CARD-0272). Deliberately different from the Pi's, not drift — see `core/docker/README.md` for which setting belongs to which host and why (CARD-0326).
