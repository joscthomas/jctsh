...
**Version:** 1.41
**Version description:** CARD-0335 — §4.1's boot-time heartbeat rule now states its applicability instead of saying "every component": the false watchdog alert needs the worst-case gap (2 × the heartbeat interval plus the reboot's downtime) to exceed the 35-minute timeout, i.e. an interval above roughly 15 minutes. 30-minute heartbeats must send a boot heartbeat; the 5-minute heartbeats (`hiking-monitor`, `air-quality-monitor`) cannot hit it and are exempt; systemd-timer heartbeats already do it with `OnBootSec=2min` (`photo-server`, `p-w-firefly`).
...
**Boot-time heartbeat (required — CARD-0333, 2026-09-24).** The 30-minute interval counts from *boot*, so on its own the first heartbeat after a reboot arrives ~30 minutes after boot — up to ~60 minutes after the previous one, past the 35-minute watchdog timeout. Any reboot (OTA, power cycle, restart button) that lands more than ~5 minutes after a heartbeat therefore raises a false `silent for 35 minutes` alert, which teaches everyone that silence alerts are noise. **Applicability:** the false alert needs the worst-case gap — 2 × the heartbeat interval, plus the reboot's downtime — to exceed the 35-minute timeout, i.e. a heartbeat interval above roughly 15 minutes. Components on the standard 30-minute heartbeat therefore **also send one heartbeat shortly after each boot**. Components on a 5-minute heartbeat (`hiking-monitor`, `air-quality-monitor`) cannot hit it and are exempt; systemd-timer heartbeats already do this (`OnBootSec=2min` in `photo-server`'s and `p-w-firefly`'s timers). The boot heartbeat has three conditions:
...
Full changelog moved to `JCTsh-Build-Standards-History.md` (CARD-0292) — read on demand only, not part of routine reading. This document's own header states the current version and description.
