...
**Version:** 1.30
**Version description:** Extended §6.4 with a Matter device registration order — commission into HA first, then expose to Google Home through HA, never register in Google Home directly or into both independently. Harvested from a real gap (the household's first 3 Matter devices registered directly in Google Home, leaving HA with zero visibility) and documents the Android Companion-app commissioning mechanics plus the Matter Server backend requirement on a Docker Container HA install (CARD-0262).
