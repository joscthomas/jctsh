...
- **Use `JCTsh-Perfboard-Build-Template.md`** (repo root) as the section skeleton for a new component's `perfboard-layout.md` — Board Note, Bus Planning, Assembly Sequence, Pre-Power Checks, and Reboot/Power-Cycle Verification are all proven-out structure, not a from-scratch design each time.
...
**Design provision for a secondary debug UART, required for any component that will lose convenient use of its onboard USB-C port for live debugging once built/deployed.** The onboard port stops being a reliable debug console whenever any of the following holds, not just the battery case:
- **Battery-powered with a separate regulator** — USB and the battery/regulator path can't safely be powered simultaneously (backfeed risk between the two regulators), so the onboard port can only show logs while running on USB power, never on the actual battery path a real boot-time or field issue happens on.
- **Going into an enclosure** — once sealed, the onboard port is no longer conveniently reachable without disassembly, regardless of power source.
- **Deployed somewhere physically inconvenient** — mounted, outdoor, or otherwise not sitting on a bench next to a PC when the behavior worth watching actually happens.
- **Debugging a WiFi/MQTT problem specifically** — OTA-based log visibility depends on the very transport that may be what's failing, so it can't be trusted as the only debug channel for that class of issue.

Reserve GPIO17 (UART2 TX) for this at design time on any component meeting one or more of the above — free of other duty, not claimed by a sensor or switch — so a debug adapter can be added later with a two-wire jumper addition, not a rework. TX-only is sufficient (`logger:` only ever transmits); RX/GPIO16 doesn't need to be reserved.
...
**2. Firmware low-battery cutoff required — and it must cover every high-current WiFi-burst operation, not just continuous field/logging operation.** Watch the battery voltage sensor. Below a safe threshold (3.4V for single-cell LiPo — leaves margin above the cell's own PCM trip point and above boost-converter end-of-charge instability), force deep sleep regardless of any mode switch, and render a persistent on-screen warning if a display is present (e-ink holds the frame with zero power). Also check at boot, so waking a critically-low device shows the warning and safely re-sleeps instead of attempting normal operation. This layer exists to act *before* the cell's PCM has to trip — it gives an early, visible warning instead of a silent hard stop. **A cutoff scoped only to the continuous logging/sensing loop is not sufficient** — any discrete, high-current burst operation (a buffered-data replay over MQTT, an OTA check, anything that drives WiFi TX for more than a moment) needs the same gate, checked immediately before the burst starts, not just periodically during idle operation. Found the hard way, 2026-08-25 (CARD-0211/CARD-0212): hiking-monitor's existing cutoff protected the hike-logging loop but was deliberately skipped while docked/charging (correctly, so charging isn't interrupted) — but the docked-mode data-replay burst had no gate of its own, so a device that came home from a hot, long hike already battery-warned went straight into repeatedly attempting (and brownout-resetting through) a multi-second WiFi replay burst before the battery had recovered any headroom. Reference: `components/hiking-monitor/hiking-monitor.yaml` (`low_battery_shutdown` script).
...
**8. [CANDIDATE — not yet required, pending validation] GPIO-controlled power gating for I2C/SPI peripherals during sleep.** Observed 2026-07-03: ESP32 deep sleep only stops the CPU from executing — it does not cut power to anything downstream. With no gating in place, I2C/SPI peripherals (BME280, LTR-390, etc.) stay fully powered and drawing their own operating current for the entire "sleep" duration, on top of any boost-converter quiescent draw (see point 7). Confirmed visually on hiking-monitor: the ESP32's and LTR-390's power-indicator LEDs stayed lit after the device entered deep sleep. Candidate fix: a P-FET (or similar high-side load switch) in-line on the 3.3V rail between the microcontroller's own 3.3V output and the peripherals — not between the boost module and the microcontroller's `VIN`, since the microcontroller must stay powered to control the gate signal — gated by a spare GPIO, cutting peripheral power during sleep and re-enabling it on wake. **Not promoted to a required standard yet.** CARD-0070 (which absorbed CARD-0027) partially validated this on a rig prototype — the LDO half of that card is confirmed working, but the gating half hit a persistent parts-quality issue (likely-counterfeit BS250 stock) and was **deferred 2026-08-14** without full end-to-end validation or being applied to the real field device. Real, useful findings from that work either way: the correct wiring topology (P-FET between the microcontroller's own 3.3V rail and the peripherals, not between the boost/LDO output and the microcontroller itself), and a hard-learned requirement — a gate pull-up resistor, or a floating gate can stay conductive through sleep even with the GPIO correctly driven high beforehand. Revisit this entry if a future build actually completes and measures the gating half.

**9. Regulator peak-current headroom and bulk capacitance at the point of load — required, with quantified numbers, not just a qualitative preference.** Added 2026-08-25 after the same underlying physics caused two separate same-night incidents on two different devices (CARD-0198 air-quality-monitor, CARD-0211 hiking-monitor): a WiFi TX/association current spike lands on a battery+regulator chain that only has enough headroom for the *average* draw, sagging the rail below the ESP32's brownout threshold and forcing a reset — sometimes before any code has run far enough to log what happened.

- **Why this keeps happening — the actual physics, not a design mistake:** WiFi association/TX draws current in sharp, millisecond-scale spikes, not a smooth average. This project's own real measurements: hiking-monitor's active-boot/WiFi current bounced **109-154mA continuously** on the CARD-0026 test rig (never settling), consistent with the general **100-250mA burst range** that rig's brownout-reset-loop diagnosis was built on; air-quality-monitor's SEN55 sensor draws **~63mA** in active Measurement mode, on top of whatever the ESP32/WiFi radio itself needs concurrently. A battery's own internal resistance also rises as it depletes, heats up, or ages — the *identical* hardware configuration can pass repeatedly on a fresh, cool cell and fail repeatedly on a partially-depleted or hot one (hiking-monitor's CARD-0211 incident: a battery that started the day at 4.28V, after a ~6.5 hour hike in 108-111°F heat, couldn't sustain a WiFi replay burst it would normally handle fine).
- **Size the regulator for the peak coincident load, not the average.** Sum every current-hungry thing that could plausibly be active at the same moment (WiFi TX/association + any actively-powered peripheral + baseline MCU draw), then pick a regulator (LDO or switching buck/boost) rated for **at least 2-3x that sum** — not a part that's only just barely rated for the theoretical peak. air-quality-monitor's MCP1700 (250mA rated) against a WiFi spike stacked on SEN55's ~63mA is exactly the marginal case this rule is meant to prevent; a regulator with real headroom turns "sometimes works, sometimes doesn't" into "just works."
- **Bulk capacitance at the point of load is a real, required complement — not a substitute for headroom.** A capacitor can only buffer millisecond-scale transients (a 1000µF cap holding up ~100mA from 3.3V down to the ESP32's ~2.4V brownout threshold lasts under 10ms by simple C×ΔV/I math) — it cannot manufacture sustained current a regulator can't supply, and testing this project did in 2026-08-24 confirmed a bulk cap alone did not reliably fix a case where the regulator itself was genuinely undersized. But for genuinely short WiFi-TX-scale spikes, it's real and effective. Place a few hundred µF electrolytic in parallel with a small (1-10µF) ceramic directly at the microcontroller's 3.3V/GND pins — as close to the point of load as physically possible, not just somewhere on the same rail.
- **Not a substitute for point 2's low-battery cutoff.** Even a well-headroomed regulator has limits — the firmware-side cutoff (point 2, now scoped to cover WiFi-burst operations specifically) is what protects a deeply-depleted cell regardless of how generous the regulator's rating is.

**10. Sequence current-hungry startup operations in firmware — don't let them coincide.** WiFi association and a peripheral's own power-up/active-mode current are each individually manageable; stacked on top of each other during the same boot window, they're what actually blows past the regulator's peak rating. Where firmware has control over the timing (most peripheral power-up does), hold a hungry peripheral in its lowest-current state until WiFi/MQTT has actually connected (not just "probably done by now" on a fixed delay — check the real connection state, bounded by a timeout so a device with no reachable network doesn't wait forever), then bring it to full power. Reference: `components/air-quality-monitor/air-quality-monitor.yaml`'s `on_boot` sequence — SEN55 is forced into ~2.6mA Idle mode as the first `on_boot` action, before the reset-reason LED check even runs, and only resumes full ~63mA Measurement mode once `mqtt_client.is_connected()` is confirmed (bounded to 30s). This pattern alone wasn't sufficient to fully solve CARD-0198 (point 9's regulator-headroom rule is what closes the remaining gap), but it's still a real, required piece of the overall mitigation, not optional once a peripheral's active-mode draw is non-trivial.

**11. A field device that buffers data for later upload must never attempt WiFi while actively collecting — only once its own session-over signal says so, never just because a charging/dock signal went high.** Added 2026-08-27 after hiking-monitor's real reset storm (CARD-0217) and the closely-related CARD-0045 finding, both traced to the same root design mistake: nothing was ever telling the WiFi radio to stay off during field/collection mode, so it kept retrying continuously — burning current for zero benefit (nothing reachable out in the field) and, once battery voltage got low enough under heat, triggering repeated brownout resets. A charging/dock signal (`dock_detect`-style, USB or solar sharing the same pads) is **not** a valid proxy for "safe or useful to try networking now" — solar in particular is used specifically for extended time away from any network, so it correlates with *worse* connectivity, not better. The correct, simple rule: WiFi only re-enables once the device's own explicit "this collection session is over" signal fires (a mode switch turned off, an equivalent state transition) — never merely because power got connected while still actively collecting. This is deliberately simpler than an earlier-considered alternative (a bounded WiFi attempt window with periodic retry while charging) — that design still tries to guess at "maybe a network is reachable now," when the honest answer during active collection is that trying is never justified, so there's nothing to bound or retry. **Applying this to a device without a readable mode signal isn't automatic — check first, don't assume one exists.** Confirmed while checking this against air-quality-monitor's own not-yet-built Step 8 (CARD-0218): that device was deliberately designed "dock-detect-only for firmware mode-switching... no GPIO-based manual mode switch" (its inline power switch is wired directly into the battery path, unreadable by firmware) — meaning it had *no* signal to distinguish a genuine end-of-session from solar connecting mid-session. **Resolved the same day, same card** — see point 12 below for the fix (a repurposed switch now serves as a real Intent signal). Reference: `components/hiking-monitor/hiking-monitor.yaml` (`dock_detect`'s `on_state` handler, gated on `binary_sensor.is_off: slide_switch`), `components/hiking-monitor/operations.md` (the corrected operational rule), CARD-0045, CARD-0217, CARD-0218.

**12. Three independent signals are required for any battery-powered field device that collects data now and syncs later — Intent, Power Connected, Power Switch — never let one stand in for another.** Added 2026-08-27, the hardware-design framework underneath point 11's firmware rule. Defined:

- **Intent** — is the device actively collecting field data right now, or is it idle/ready-to-upload? A deliberate, user-set declaration (a physical switch position, an explicit mode command) — never inferred from anything else. Point 11's whole rule depends on this signal existing and being independently trustworthy.
- **Power Connected** — is external charging current present right now (USB or solar)? A pure power-supply fact. Says nothing about intent, and nothing about whether the device is done collecting.
- **Power Switch** — can the device reach a genuine zero-draw off state (for storage/transport/shipping), not just its lowest *operating* power mode?

These three are orthogonal by nature — real, independent facts that vary in any combination (actively collecting *and* charging via solar mid-hike; done collecting *and* not yet charging, waiting to be docked later). Conflating any two of them — using one as a stand-in for another because it usually happens to correlate — is the exact root cause behind every bug this session found: hiking-monitor's real reset storm (CARD-0217) and CARD-0045 both came from inferring "safe to try WiFi" (an Intent-gated decision) from Power Connected instead of a real Intent signal.

**A real audit of both existing field devices, 2026-08-27 — neither has all three, independently, and each is missing a different one:**
- **hiking-monitor** has Intent (slide switch, GPIO27) and Power Connected (`dock_detect`, GPIO32) cleanly separated — correctly designed, which is exactly why CARD-0045/CARD-0217's firmware fix could be applied cleanly once found. But it has **no real Power Switch** — per `operations.md`, "VOUT+ runs directly to ESP32 VIN — the switch is not in the power path"; the slide switch is pure Intent, and the only true off state is physically disconnecting the LiPo's JST connector, which requires opening the sealed enclosure. Tracked as **CARD-0181** (Defer).
- **air-quality-monitor** had a real Power Switch (an inline cutoff wired directly into the battery path — genuine true-off, unlike hiking-monitor) and Power Connected (`dock_detect`), but **no Intent signal at all** — its 2026-08-19 design decision built mode-switching entirely around `dock_detect`, conflating Power Connected with Intent from the start. Surfaced and resolved same-day, 2026-08-27 (**CARD-0218**), while correcting Step 8's design to match point 11: since this device's existing switch (Gebildet SS12D10, slide type) was already occupying the Power Switch role, the fix is the mirror image of hiking-monitor's — that switch is repurposed as the real, GPIO-readable **Intent** signal (GPIO27, same pin/pattern as hiking-monitor's own), and Power Switch gets a new, genuinely distinct part instead (a BK-1208 latching push button, the same part chosen for hiking-monitor's own CARD-0181 gap — one order covers both devices). Design decided and documented (`air-quality-monitor-claude-code-instructions.md`), not yet physically rewired on the breadboard or ordered.

**Required for every new field-and-later-upload device going forward:** design and wire all three signals independently and explicitly from the start, even when two of them seem to move together in the common case during early bring-up. The failure mode only shows up once they genuinely diverge (solar connecting mid-session, a device left disconnected a while) — easy to miss in initial testing, expensive to find later (a real multi-hour field incident, not a bench test).

**13. The complete data-flow model for a field-and-later-upload device is three distinct steps, not one — collecting, attempting, and succeeding are separate states with separate gates.** Added 2026-08-29, clarifying the full picture behind point 11's rule.

- **Step 1 — the attempt gate is an AND, not either signal alone: Intent off *and* Power Connected both required.** Power Connected is the literal trigger — nothing is attempted until it goes HIGH — but it is not *sufficient* by itself (that was the point 11 bug). Intent-off is the second, independently-required condition added on top of it. Neither replaces the other; both must hold.
- **"Power Connected" is deliberately broader than the word "docked" suggests — don't let the word narrow the mental model.** The signal is defined purely electrically (voltage present on the charging input) and fires identically whether that power came from the home charging dock, a solar panel, or a field USB power bank. This breadth is intentional, not incidental — it's what lets a device reach WiFi from a solar-charged trailside stop, not just at home. Referring to this condition as "docked" in design discussion risks quietly reintroducing the assumption it only applies at home, which is the exact class of mistake point 11 exists to prevent.
- **Step 2 — meeting the attempt gate starts a bounded attempt, not a guaranteed upload.** The device tries for a bounded window, then backs off and retries periodically for as long as both gate conditions still hold, rather than either trying indefinitely or giving up permanently on one failure. (Reference implementation: `air-quality-monitor.yaml`'s Timeout policy — ~2 minute attempt window, ~15-20 minute retry interval; treat the *pattern* as the requirement, not these exact numbers for every device.)
- **Step 3 — the actual upload/replay only begins once WiFi *and* MQTT both succeed** — a separate, later condition from "the attempt gate was met." A device can be Intent-off and Power-Connected for a long stretch with no network in range and never upload anything, which is expected, not a fault. Once connected, this is also when a periodic heartbeat (5 min, both current field devices) starts publishing alongside the actual data replay.
- **Whether field-mode logging itself keeps running while Power Connected is HIGH is a separate design choice, not dictated by this point.** hiking-monitor stops field logging the instant Power Connected goes HIGH; air-quality-monitor runs it unconditionally regardless of Power Connected state, specifically because solar/field-USB made "connected" no longer synonymous with "session is over." Either is valid — decide deliberately per device rather than copying the other's choice by default.
...
**Cross-device unique_id collisions — check this before naming any discovered entity.** ESPHome's default `discovery_unique_id_generator: legacy` derives an entity's MQTT discovery `unique_id` from its type and *display name alone* (e.g. a button named "Restart" becomes `ESPbuttonrestart` on every device that has one) — not from the device's own identity. Two devices with an identically-named entity silently collide: HA keeps whichever one registered first and drops the other with no visible error to the user (only a `logger.error` line: `"ID ... already exists - ignoring ..."`). This is exactly how hiking-monitor's, front-porch-temp-sensor's, and salt-sensor's restart buttons collided with each other (CARD-0180/CARD-0186) — salt-sensor's had never worked in HA at all, unnoticed until directly investigated.

Before adding any discovered entity, grep the other components' YAML for the same generic entity name (`"Restart"`, `"Status"`, etc.) to check for a collision. Two fix patterns, pick based on blast radius:
- **Device exposes only that one entity to HA** (everything else `internal: true`) — set `discovery_unique_id_generator: mac` on the device's `mqtt:` block. Safe because it's the only entity whose id changes.
- **Device exposes multiple live entities to HA** — do *not* flip the generator; it regenerates every entity's unique_id on that device at once, orphaning existing HA registrations (history, dashboard references, automations) for entities that were never part of the problem. Instead give just the colliding entity a device-specific `name:` so the existing legacy generator naturally produces a distinct id.

Either fix path leaves a stale retained discovery topic behind (the old topic keeps publishing the old, collided config). Clear it explicitly — `mosquitto_pub -t '<old discovery topic>' -n -r` — or HA will show a duplicate/orphaned entity alongside the fixed one. Also avoid baking the device's own name into an entity's `name:` as the "device-specific" fix (e.g. naming a button "Salt Sensor Restart" on the salt-sensor device) — HA concatenates device name + entity name for the default entity_id/friendly name, so this produces a doubled, awkward result (`button.salt_sensor_salt_sensor_restart`). Prefer a name that's distinct without repeating the device name, or accept the cosmetic duplication and rename the entity_id manually in HA afterward.
...
2. **Matter, commissioned into HA first, then exposed to Google Home through HA** — most new devices ship Matter-capable. Check whether it's Matter-over-WiFi (joins the LAN directly, no extra hardware) or Matter-over-Thread (needs a Thread Border Router somewhere on the network — several existing devices, including SmartThings' own hub, can serve this role without the device itself needing to live in SmartThings). **Commission into HA before Google Home, not the reverse or both independently** — see the dedicated note below; this ordering was learned the hard way from the first real Matter devices.
...
**Matter device registration order, added 2026-09-12 (CARD-0262) — a real gap found, not a hypothetical.** The household's first three Matter devices (Cync under-cabinet lights) were registered directly in Google Home. Result: zero visibility from HA (`404 Entity not found` on every one, confirmed live) — Google Home doesn't sync back to HA the way SmartThings did. **Always commission a new Matter device into Home Assistant first**, then expose it to Google Home via HA's own native Google Assistant integration (§6.4 point 2 above) — never register in Google Home directly, and never commission into both independently (Matter's multi-admin support makes this technically possible, but it creates two separate registrations of the same device that can drift, rather than one source of truth with Google Home as a view into it). This mirrors the exact pattern already used for every JCTsh-custom device reaching Google Home.

**Commissioning mechanics on Android** (this household's phones): the official Home Assistant Companion app (Play Store build, not F-Droid) has an "Add Matter Device" flow that hands off to Android's own system-level Google Matter commissioning UI to do the actual pairing, then relays the result back to HA in the background — needs the Companion app's Location permission set to "Allow all the time." This is a *front-end* only — HA's own Matter integration and its backend Matter Server (`python-matter-server`) must already be configured for this to have anything to hand the device off to. On a plain Docker Container HA install (not Home Assistant OS), that backend doesn't come for free — it needs its own container, added explicitly, not just an "Add Integration" click. See CARD-0262 for the concrete setup.

**Real gotcha found during CARD-0262's build, worth checking on any future Matter device: `python-matter-server` needs `--primary-interface` pinned on a dual-homed host.** The Pi runs both `eth0` (wired, its normal primary) and `wlan0` (WiFi) — with neither pinned, the server logs `Using 'None' as primary interface (for link-local addresses)` at startup, and commissioning a WiFi-only Matter device fails with `PASESession timed out` / `Secure Pairing Failed` against the device's link-local IPv6 address, since the server doesn't know which interface to send that traffic out on. Fix: add `--primary-interface wlan0` (or whichever interface the target device actually joins) to the container's `command:` in `core/homeassistant/docker-compose.yml` — confirmed via the startup log correctly switching to `Using 'wlan0' as primary interface`, and by three subsequent commissioning attempts all succeeding with the link-local address correctly carrying the `%wlan0` zone identifier. Only relevant on a host with more than one active network interface — a single-homed HA install wouldn't hit this.

### 6.5 New Automations: HA vs. Google Home, Never a New SmartThings Routine

Decided 2026-09-11 (CARD-0164), extending §6.4 from devices to automations. Three automation engines exist in this household — Home Assistant/Node-RED, Google Home's own Routines, and SmartThings' own Routines — and the choice between the first two is a single test, with the third no longer a candidate for new work:

**The test: does Robin need to see, trigger, or adjust this herself?**
- **Yes → Google Home Routines.** She uses Google Home daily and doesn't use HA or SmartThings directly (and likely never will) — it's the only one of the three automation surfaces she can actually touch. Simple, voice-relevant automations belong here even though Google's own Routines are less capable than the other two.
- **No → Home Assistant / Node-RED.** JCTsh's own internal logic — multi-condition state machines, sensor fusion, timers, anything that should be version-controlled in this repo and observable on the log dashboard — belongs in HA/Node-RED, already the established "brain" per this project's own architecture (`CLAUDE.md`'s Component Roles table). This is the default for anything that isn't specifically Robin-facing.

**SmartThings Routines are not a destination for new automations, regardless of which side of the test above an automation falls on.** Not because they're technically deficient — they're arguably more capable than Google's own Routines — but every reason to use them evaporates once new work isn't building on SmartThings by default: Robin can't see or adjust them, they get none of JCTsh's own observability, and anything triggered from HA re-creates the exact paid-API dependency §6.4 exists to stop growing. **Existing native SmartThings Routines** (a real SmartThings device triggering another real SmartThings device, with HA never in the loop) are fine to leave running untouched — they don't depend on the deprecated developer API at all — this rule is about where *new* automation logic goes, not a mandate to migrate what already works.

**A real gotcha when moving an existing automation off a SmartThings-sourced device:** Google Home identifies a device by which partner integration supplied it. Swapping a device from SmartThings-sourced to HA-sourced — even keeping an identical entity_id inside HA — makes Google treat it as a brand-new device, not the same one. Any existing Google Home Routine referencing the old device by name will silently point at a dead device afterward; it does not auto-follow. Audit Google Home's own Routines list for references to a device before migrating it, and re-point anything found afterward — this can't be checked or fixed from outside the Google Home app itself.
...
**Reference implementation:** `components/photo-server/backup.md`, `hosts/m8/network.md`.
