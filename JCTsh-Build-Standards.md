...
**Version:** 1.22
**Version description:** Added §2.14 points 9-10 — quantified regulator peak-current headroom (2-3x the coincident peak load, not the average) and required bulk capacitance at the point of load, plus a firmware-sequencing requirement to avoid stacking WiFi association with a peripheral's own power-up current. Extended point 2's low-battery cutoff to explicitly cover WiFi-burst operations, not just continuous logging. Harvested same-night from CARD-0198 (air-quality-monitor) and CARD-0211/CARD-0212 (hiking-monitor) hitting the identical brownout-under-WiFi-spike physics on two different devices.
