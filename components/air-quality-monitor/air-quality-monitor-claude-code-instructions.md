...
**Two open items, not yet decided — resolve before writing this gate in code:**
1. **Is the new upload-safe threshold a second, separate check layered on top of the existing generic low-battery shutdown cutoff (device still fully shuts down at 3.4V regardless of networking; uploads specifically also require clearing a higher bar, e.g. 3.6-3.7V), or does it replace the shutdown cutoff's number for this device specifically?** Leaning toward "separate and additional" — keeps the general safety floor unchanged and adds a stricter bar just for the higher-current upload action — but this is Joseph's call, not decided yet.
