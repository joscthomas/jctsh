...
**Version:** 1.3
**Version description:** Step 8's full design finalized 2026-09-09 (see Step 8 below): the boot-time WiFi-gate mechanism (including the priority-ordering finding that this device's `on_boot` — priority -100 — can safely call `wifi.disable()` directly, unlike hiking-monitor's priority-600 block, which had to defer to the interval), the bounded-attempt/retry state machine, SEN55's duty-cycle mode-switching timing (confirmed `SEN5X_CMD_START_MEASUREMENTS_RHT_ONLY = 0x0037` against the installed ESPHome source), the LED diagnostic scheme (Blue dedicated to networking status), and the MQTT logging plan (buffered vs. live per state) are all specified in full — ready to implement, not yet written to `air-quality-monitor.yaml`.
**Version 1.2 description (2026-08-19 to 2026-08-27):** Step 1 resolved (2026-08-19, Joseph): dock-detect-only for mode-switching confirmed, plus a new inline power switch (wired directly into the battery path, no GPIO) added for true transport/storage off — kept deliberately separate from mode-switching, per the CARD-0181 lesson that a GPIO-tapped switch only sets a mode flag rather than cutting power. Power architecture also changed from the originally-planned TP4056+boost combined module to a direct LiPo-to-LDO design (MCP1700) per `JCTsh-Build-Standards.md` §2.14 point 7 — recalculated runtime accordingly (see Hardware Context). **Revised 2026-08-27 (CARD-0218), resolving Step 8's Intent-signal gap:** this device now gets a real Intent switch after all — the existing Gebildet SS12D10 slide switch is repurposed from Power Switch duty onto GPIO27 (previously unused) as a GPIO-readable Intent signal, matching hiking-monitor's own pattern exactly; a new BK-1208 latching push button takes over the Power Switch role, inline in the battery+ path where the slide switch used to sit. See `JCTsh-Build-Standards.md` §2.14 point 12 for why these need to be two distinct physical controls, not one.
...
| Inline power switch | **Revised 2026-08-27 (CARD-0218) — role swapped from the slide switch to a new BK-1208 latching push button** (2-pin, DC 30V 1A, 12×8×8mm — same part chosen for hiking-monitor's CARD-0181; **ordered 2026-08-28**, one order covers both devices, not yet on hand/arrived). **Not a build blocker** — a spare SS12D10 (slide switch, Bin A3 assortment) stands in for this role on the breadboard until the BK-1208 arrives, same wiring/node, swapped in later with no other rewiring. Wired **directly into the battery+ path**, ahead of the TP4056/regulator, no GPIO involved — same position the original slide switch used to occupy. True hard off for transport/storage. Per `JCTsh-Build-Standards.md` §2.14 point 12: Power Switch and Intent need to be two genuinely distinct physical controls, not just electrically separate — the repurposed slide switch (below) is now Intent, so Power Switch needed its own, differently-feeling control rather than staying a second slide switch — the substitute slide switch here is a temporary stand-in, not a reversion of that decision. Pre-satisfies `JCTsh-Build-Standards.md` §1.7 before enclosure design even starts. |
| Intent switch | **New, 2026-08-27 (CARD-0218) — resolves Step 8's previously-open "no Intent signal" gap.** The existing Gebildet SS12D10 slide switch (SPDT, wired as SPST — Bag 23, on hand) is repurposed here, off battery-cutoff duty, wired instead as a plain GPIO-read digital input on **GPIO27** (previously unused, freed up when the SEN55 power-gate was dropped at Step 6) — same pin role and wiring pattern as hiking-monitor's own slide switch (GPIO27, internal pull-up, inverted logic). Answers "collecting field data, or idle/ready-to-upload" — the signal `dock_detect` was incorrectly standing in for until this decision. |
...
| GPIO27 | **Intent switch** (added 2026-08-27, CARD-0218) — the repurposed slide switch, same pin/pattern as hiking-monitor's own Intent switch. Previously reserved for the SEN55 power-gate transistor base, dropped 2026-08-21 (see Hardware Context table above), then briefly unused until this assignment. |
| GPIO18 | RGB LED — Red |
| GPIO19 | RGB LED — Green |
| GPIO23 | RGB LED — Blue |

**Step 1 — resolved 2026-08-19 (Joseph), revised 2026-08-27 (CARD-0218):** dock-detect-only for firmware mode-switching is **no longer the design** — superseded once the three-signal gap was found (see the Step 8 note below for the full story). Current design: a real GPIO-readable Intent switch (the repurposed slide switch, GPIO27) plus a separate inline Power Switch (the new BK-1208 push button, no GPIO) — see the Hardware Context table above for both. This still keeps the same two concerns (which firmware mode vs. is the device powered) deliberately separate, per the original CARD-0181 lesson — it just now uses a real switch for the first one instead of `dock_detect`, which was never a valid stand-in for it.
...
**Resolved 2026-08-19, revised 2026-08-27 (CARD-0218) — see Step 1 above and the Step 8 note below.** Dock-detect-only for firmware mode-switching is no longer the design. A real GPIO-readable Intent switch (repurposed slide switch, GPIO27) plus a separate inline Power Switch (new BK-1208 push button, no GPIO) now cover the two concerns this section originally split — see Hardware Context above for the full reasoning. GPIO table updated to add the GPIO27 row.
...
**Unblocked 2026-09-08 — CARD-0198 closed.** The power design is now settled and validated: Pololu D24V10F3 regulator swap and CARD-0218's Intent/Power-switch redesign are both physically built, and CARD-0198's staged `minimal-test.yaml` sequence confirmed both of the device's real operating modes (field: battery + SEN55 alone; docked: TP4056 + battery + WiFi/MQTT/SEN55) stable. Two real findings from that testing carry directly into this step, below: the power switch needed rewiring to actually do its job, and the low-battery cutoff needs real margin. The text below reflects the target hardware; part names and wiring have been updated accordingly.

**Power switch rewiring, added 2026-09-08 (CARD-0198's own finding, folded in here rather than a separate card since Build is already in progress).** The switch's real job is to let a person force a full cold restart on demand (not primarily storage/transport off, which is a bonus) — but CARD-0198 measured that the original wiring (switch directly in the LiPo's own leg, upstream of a node TP4056 could also feed from USB) fails at exactly that job whenever docked: TP4056's own charge output backfeeds the shared node regardless of switch position, so switching off while docked doesn't actually remove power from the ESP32. **Rewire so the switch gates only the Pololu's `VIN`**, with LiPo `BAT+` and TP4056 `BAT+` tied together directly upstream of it — see `wiring.md`'s "Inline Power Switch" section for the full new diagram and reasoning. Side benefit: charging now works even with the device switched off, which it couldn't before.

**Claude Code does:**
Document the JST polarity verification procedure (same requirement as hiking-monitor) before first battery connection. Include confirming the Pololu D24V10F3's `VOUT` reads a clean 3.3V under load and the BK-1208 power switch (now wired per the rewiring above, gating only the Pololu's `VIN`) actually breaks the circuit in both positions — **measure for zero voltage/current at the Pololu's `VIN` pin with the switch off, both with USB unplugged from TP4056 and with USB plugged in.** The USB-plugged-in check is the one CARD-0198's original verification instruction missed — it would have passed under the old wiring with USB unplugged and silently failed with it plugged in, exactly the gap that testing found the hard way. Also confirm TP4056's charge LED still indicates charging with the switch off and USB connected, direct proof charging no longer depends on switch position. Note that this switch is what satisfies `wiring.md`'s "never power from USB and the regulator at the same time" caution — switching it off removes the regulator's `VIN` entirely, equivalent to unplugging it, so USB flashing just requires the switch off rather than any physical disconnect. This isn't a one-time Step 7 concern: once the battery/regulator are wired in here, it applies to every subsequent USB connection for the rest of the build and field life of the device — Step 8's and Step 9's USB flashes below, and any future re-flash over USB, all need the switch off first.

**Dock-detect, battery-divider, and Intent-switch raw checks belong here, not Step 8.** All three signals were wired before Step 8 needs them, but none has been checked on its own — dock-detect and the Intent switch would otherwise only get exercised buried inside Step 8's much more complex WiFi-attempt/gating logic, and the battery divider is still reading off the 3.3V rail placeholder rather than the real LiPo. Step 7 is when the real battery goes in and USB gets connected/disconnected anyway, so it's the cheap, isolated place to confirm all three raw signals before Step 8 adds behavior on top of them.

**Joseph does:**
Verify polarity, connect the EEMB LiPo pouch to the TP4056 module, confirm normal charge behavior (TP4056 side, now upstream of the switch) and normal 3.3V power delivery via the Pololu D24V10F3 regulator (not the boost module's old 5V→onboard-regulator path). Confirm the BK-1208 power switch cuts power cleanly in both directions — including the USB-plugged-in check above. **Battery divider:** first **rewire R1's top leg from the 3.3V rail placeholder to the real LiPo `BAT+` (post-switch, i.e. the Pololu `VIN` side)** — per `wiring.md`'s Battery Voltage Divider Wiring section, this move was always deferred to "when the power system is integrated," which is now — then compare the `battery_v` sensor's logged value (heartbeat or serial log) against a multimeter reading taken directly at the battery terminals to confirm they're close (the 100kΩ/100kΩ divider's `multiply: 2.0` filter should reconstruct the true battery voltage). **Dock detect:** connect and disconnect USB (or briefly bridge the divider's input node to 3.3V, if easier) while watching the serial log during an `esphome run` session — confirm `dock_detect`'s logged state flips HIGH/LOW as expected, independent of any WiFi-attempt behavior (that's Step 8's concern, not this check's). **Intent switch:** toggle the SS12D10 (now on GPIO27) while watching the serial log — confirm it reads LOW when closed/ON and HIGH (via internal pull-up) when open/OFF, independent of any WiFi-gating behavior (also Step 8's concern, not this check's).

**Joseph confirms:**
Polarity correct, battery connected, regulator delivering clean 3.3V, BK-1208 switch verified to actually cut power in both positions with USB both unplugged and plugged into TP4056 (not just visually "off"), TP4056 charge LED confirmed still active with the switch off, everything functioning normally. Battery divider reading confirmed close to a direct multimeter measurement. Dock-detect confirmed flipping HIGH/LOW correctly in the serial log on USB connect/disconnect. Intent switch confirmed flipping LOW/HIGH correctly in the serial log on toggle.
...
**Corrected 2026-08-27 (CARD-0218), superseding the bounded-retry design originally planned here — see `JCTsh-Build-Standards.md` §2.14 point 11. This surfaces a real, unresolved architecture gap for this specific device, not just a design tweak.** Dock-detect HIGH must not trigger a WiFi attempt on its own — dock-detect is shared with solar (same IN+/IN- pads as USB), and solar connecting mid-session says nothing about network availability, so trying is never justified while still actively collecting, full stop, same as hiking-monitor's corrected rule.

**Resolved 2026-08-27, same day this was flagged — this device now has a real Intent signal, not just a diagnosis of the gap.** Interviewed through several switch-design candidates (guarded/recessed toggle — too bulky/enclosure-complex; mini rotary — solid option, superseded) before landing on the same part chosen for hiking-monitor's own missing-signal gap (CARD-0181): a **BK-1208 latching push button** takes over the Power Switch role, freeing the existing SS12D10 slide switch to become a real, GPIO-readable **Intent switch on GPIO27** — matching hiking-monitor's exact pin/pattern. `wifi.enable()` in Step 8's firmware should be gated on this Intent switch being off (session genuinely over), exactly like hiking-monitor's corrected `dock_detect` handler above — never on `dock_detect` alone. See the Hardware Context table's "Inline power switch" and "Intent switch" rows for the full wiring detail, and `JCTsh-Build-Standards.md` §2.14 point 12 for why two distinct physical controls were needed rather than one.

**Third gating condition added 2026-09-08, from CARD-0198's live testing — "sufficient power," not just "network available."** Intent-off + Power-Connected-true was always meant to answer "is a network reachable," but CARD-0198 found that's not the same question as "is it safe to actually attempt the upload." A real battery-only trial degraded into a self-sustaining brownout-reset loop once voltage sagged to the Pololu's documented 3.4V floor — the same number the standard low-battery firmware cutoff already used, leaving zero real margin by the time it fired. **`wifi.enable()` must therefore also require battery voltage above a new, real-margin threshold**, meaningfully higher than 3.4V. All three conditions (Intent off, Power Connected true, voltage above the upload-safe margin) are required together; any one false means keep buffering, don't attempt.

**One item resolved, one still open:**
1. **Decided 2026-09-08 (Joseph): separate and additional, not a replacement.** The existing generic low-battery shutdown cutoff (§2.14 point 2, protects the LiPo cell from over-discharge) is unchanged — field mode keeps collecting and buffering at low current all the way down to that floor, same as any other JCTsh device. The new upload-safe threshold is a second, higher, narrower gate that applies only to attempting WiFi/MQTT — a device sitting below the upload threshold but still above the general safety cutoff just keeps logging and buffering, waiting for voltage to recover (get docked/charged) or for the general cutoff to eventually shut it down. It's never forced to stop collecting early just because uploading isn't currently safe.
2. **Provisional value set 2026-09-08: 3.8V — not yet confirmed by a real bench sweep, treat as a placeholder.** Estimated from the data on hand rather than measured directly: 3.4V (measured mid-loop) is confirmed catastrophic — zero regulator headroom, a sustained non-recovering brownout loop; ~3.86-3.88V (2026-08-28, battery alone) still showed occasional single-brownout-then-recover on WiFi association, not fully clean; TP4056-alone (zero battery contribution) was rock-solid 4/4 clean across all three combined-load stages today, suggesting real spare capacity once docked. 3.8V sits meaningfully above the catastrophic floor and errs toward the safer side of the asymmetry: an underestimate risks a real, hands-on-intervention-required lockup (worse) versus an overestimate only costing delayed uploads (milder) — see this card's own discussion for the full reasoning, folded in here as design rationale. Confirm with a real bench sweep at the first practical opportunity (trigger a WiFi+SEN55 burst at several voltages on the rewired hardware, find where it actually stops brownouting) — same empirical method CARD-0198 used for everything else in this investigation — and revise this number once real data exists; a real device's first natural approach to this threshold in the field is also a free data point worth capturing. **Caution, found 2026-09-08 during Step 7's own verification:** every voltage reading (bench sweep or runtime) must be taken with the charger disconnected — a battery's terminal voltage reads artificially high while TP4056 is actively charging it (the charging current itself elevates the reading against the battery's own internal resistance), so a "voltage" logged while docked doesn't reflect the battery's true state of charge. Confirmed live: 3.9V measured at the switch's input while the charge LED was lit, but unplugging USB and letting the battery actually supply the Pololu dropped `VOUT` to 3.18V — the real, load-bearing voltage was well below what the mid-charge reading suggested.
...
**CARD-0218 (2026-08-27): the real `/data` payload built here must include `temp_f`/`humidity_pct` from the SEN55's own `sen55_temp`/`sen55_humidity` sensors**, not just PM/VOC/NOx/battery — those two fields exist specifically so this device's readings can serve as an independent reference to compare against hiking-monitor's BME280 (mounting/placement may make this device's reading more accurate) — pointless if they never leave the device. Same `source: air-quality-monitor` / field-naming convention every other source already uses in the Environmental Data schema, no special-casing needed. `/data` field set (Environmental Data Schema): `temp_f`, `humidity_pct`, `pm1_ug_m3`, `pm25_ug_m3`, `pm4_ug_m3`, `pm10_ug_m3`, `voc_index`, `nox_index`, `battery_v`, `rssi_dbm`, `lat`/`lon` always null (no GPS hardware).

**Design finalized 2026-09-09, working through the mechanics in detail before writing code — five pieces:**

**1. SEN55 duty-cycle mode-switching.** 2-minute cycle (matches this project's standard sensor cadence). Idle in RHT/Gas-Only mode (`SEN5X_CMD_START_MEASUREMENTS_RHT_ONLY = 0x0037` — confirmed directly from the installed ESPHome package's `sen5x.cpp` source, not guessed) for most of the cycle; ~40s before each reading (within Sensirion's recommended 30-60s warm-up window), switch to full Measurement mode (`0x0021`), read PM/VOC/NOx/temp/humidity, then drop back to RHT-only. The warm-up wait is implemented as a loop of short delays with `App.feed_wdt()` fed each iteration (same discipline as `hike_log_replay_stream`'s per-line feed, CARD-0211) — never one giant blocking `delay()`, which risks tripping the task watchdog.

**2. WiFi never gets a free pass at boot — same latent gap CARD-0217 found and fixed on hiking-monitor.** ESPHome's `wifi:` component tries to associate automatically and unconditionally during its own `setup()` — nothing in this file today tells it not to, exactly hiking-monitor's pre-CARD-0217 state. **This device gets the fix at boot, not deferred to the first interval tick like hiking-monitor needed to.** Checked the actual priority ordering rather than copying hiking-monitor's pattern blind: hiking-monitor's gate-setting `on_boot` block is priority 600 (runs *before* WiFi's fixed `setup_priority` of 250, so `wifi.disable()` there would be unsafe — hence its deferral to the interval). This file's existing `on_boot` block is priority **-100** — since -100 < 250, WiFi's `setup()` has already completed by the time this block runs. The three-condition gate check (Intent off AND Power Connected AND battery ≥ 3.8V) becomes the literal first action in this `on_boot` block, before the existing SEN55-idle/LED sequence — `wifi.disable()` fires within milliseconds of boot if the gate isn't met, not after the ~60-80s the existing SEN55/LED/bounded-wait sequence would otherwise take. **The existing bounded 30s MQTT-wait loop only runs when the gate is true** — skipped entirely when it's false, since there's no legitimate attempt in progress to wait for.

**3. Bounded attempt window + retry backoff (`JCTsh-Build-Standards.md` §2.14 point 13, Step 2) — reference numbers: ~2min window, ~15min retry, both soft targets checked at 2-min granularity** (reusing the existing 2-min interval tick for this state machine too, same precedent as CARD-0224's periodic recheck on hiking-monitor — "this tick already runs every 2min regardless of mode, so it's the natural place to re-check... rather than a separate timer"). Two timestamps + one boolean (`wifi_attempt_start_ms`, `wifi_last_attempt_end_ms`, `wifi_attempt_in_progress`), checked every tick:
   - Gate false → `wifi.disable()` immediately if enabled, clear all attempt state.
   - Gate true, MQTT connected → nothing to do, `on_connect` already took over.
   - Gate true, MQTT not connected, no attempt tracked → start one: either a genuinely new opportunity (`wifi.enable()`, stamp start time) or *adopting* WiFi's own already-running boot-time attempt (stamp start time without re-calling `wifi.enable()`, since it's already on).
   - Gate true, attempt in progress, window not yet elapsed → wait.
   - Gate true, attempt in progress, window elapsed (~2min) → `wifi.disable()`, stamp end time, clear in-progress flag.
   - Gate true, no attempt in progress, previous attempt ended → wait for ~15min since that end time, then loop back to "start one." No cap on retry count — only on each individual window's duration, per the instructions doc's existing Timeout policy text above.

**4. LED diagnostics — Blue is the dedicated networking-status channel, since PM2.5's threshold indicator never uses it and Red/solid-Blue are already spoken for by the reset-reason check.** Decided 2026-09-09, dropped the "not attempting" case (Intent on, or no power — the common/expected case, low diagnostic value against the cost of a blink every boot):
   | State | Pattern | When (elapsed since power-on) | Duration |
   |---|---|---|---|
   | Attempting blocked — docked, Intent off, but battery < 3.8V | blue+red together, 3 blinks | first action in `on_boot`, effectively immediately | ~1.8s |
   | Actively attempting WiFi/MQTT (gate true) | slow blue blink, ~1/sec | starts once the bounded-wait section is reached (~7-10s in, after the existing reset-reason + self-test sequence) | up to 30s, or shorter if it connects sooner |
   | Connected | solid blue | whenever it succeeds within the window | ~1s |
   | Gave up, retrying in ~15min | 2 quick blue blinks | at the 30s mark, if it never connected | ~0.6s |

   Practical guidance for watching the device at the bench: the voltage-blocked pattern (if it fires) happens in the **first ~2 seconds** — easy to miss if not already watching. If nothing blue happens in that window, the gate was true; watch for the slow blink starting **~7-10 seconds** in, resolving to either solid blue or the double-blink within the next 30 seconds.

**5. Matching MQTT log messages — buffered for three of the four states, live for one.** The voltage-blocked, attempting-start, and gave-up-timeout states all occur before (or without) a connection existing by definition, so they can't be published live — they're buffered via the flash-log mechanism (same as CARD-0195's skip/reset events on hiking-monitor: `{"event":"wifi_attempt_blocked", "battery_v":...}` / `{"event":"wifi_attempt_timeout"}`, replayed alongside real readings once a connection eventually happens, requiring a matching Node-RED routing addition to `environmental-data.flow.json`'s existing "Route skip/reset events" function node). Only "connected" can log live — extend the existing `on_connect` message.
...
Then transfer the validated breadboard circuit to perfboard: ESP32 + SEN55/adapter + Pololu D24V10F3 regulator (+ bulk caps) + BK-1208 power switch + SS12D10 Intent switch + debug UART header + RGB LED + both dividers. No gate transistor (dropped Step 6). Continuity-check before power-on. Switch off before connecting USB for any post-transfer USB flash/debug, same as Step 8.
