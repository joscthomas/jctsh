...
| SEN55 power gate | BC547B NPN transistor (Music Response bin) as a low-side switch, duty-cycling the SEN55 on/off to manage its ~70mA active draw — same substitution-for-purchased-part pattern as remote-temp-sensor-01's BC557B. Correct topology for SEN55 specifically since it's a 5V peripheral (via the adapter's own onboard boost, see Power row below) rather than an ESP32-3.3V-rail I2C device — **not** the §2.14 point 8 P-FET pattern, which targets peripherals sharing the ESP32's own 3.3V rail and remains unvalidated/candidate-only regardless. No other sensor on this build to gate that way. |
...
**Joseph confirms:**
Polarity correct, battery connected, LDO delivering clean 3.3V, inline switch verified to actually cut power (not just visually "off"), everything functioning normally.
