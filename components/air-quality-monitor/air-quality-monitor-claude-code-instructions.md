...
2. **The exact threshold value isn't set.** Needs a real bench sweep (trigger a WiFi+SEN55 burst at several voltages on the rewired hardware, find where it actually stops brownouting) rather than a guessed number — same empirical method CARD-0198 used for everything else in this investigation. Do this once Step 7's switch rewiring is physically done, before writing Step 8's firmware. **Caution, found 2026-09-08 during Step 7's own verification:** every voltage reading for this sweep must be taken with the charger disconnected — a battery's terminal voltage reads artificially high while TP4056 is actively charging it (the charging current itself elevates the reading against the battery's own internal resistance), so a "voltage" logged while docked doesn't reflect the battery's true state of charge. Confirmed live: 3.9V measured at the switch's input while the charge LED was lit, but unplugging USB and letting the battery actually supply the Pololu dropped `VOUT` to 3.18V — the real, load-bearing voltage was well below what the mid-charge reading suggested. Any bench sweep step (and any future runtime voltage check this device's firmware ever does) needs to read voltage under real discharge, charger disconnected, not mid-charge.
