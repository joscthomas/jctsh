...
| Inline power switch | **Revised 2026-08-27 (CARD-0218) — role swapped from the slide switch to a new BK-1208 latching push button** (2-pin, DC 30V 1A, 12×8×8mm — same part chosen for hiking-monitor's CARD-0181, not yet on hand, order covers both devices). Wired **directly into the battery+ path**, ahead of the TP4056/LDO, no GPIO involved — same position the slide switch used to occupy. True hard off for transport/storage. Per `JCTsh-Build-Standards.md` §2.14 point 12: Power Switch and Intent need to be two genuinely distinct physical controls, not just electrically separate — the slide switch (below) is now Intent, so Power Switch needed its own, differently-feeling control rather than staying a second slide switch. Pre-satisfies `JCTsh-Build-Standards.md` §1.7 before enclosure design even starts. |
