...
- **Check 5 (Intent switch): both directions confirmed 2026-09-16, closed.** ON via debug UART; OFF via the Environmental Data Sheet once the UART adapter developed an unresolved fault mid-session (not wiring or firmware — isolated to the adapter itself or a connection internal to it, no spare on hand to confirm further).
...
- **Step 9 checklist is now fully complete.**
...
5. **Intent switch — both directions confirmed 2026-09-16, closed.** ON case: watched via debug UART (COM9), produced `DUTY CYCLE: switching to Measurement mode` at the next tick. Immediately after, the debug adapter's RXD line went permanently dark (no bytes at all, board itself confirmed still running via a steady power LED throughout) — ruled out, in order: log-capture stdout buffering (fixed with `python -u`/`PYTHONUNBUFFERED=1`, confirmed working via the check-4 retest before this happened), a stale process/serial handle (killed and restarted fresh, no change), USB re-enumeration (adapter's `DeviceID` unchanged), the adapter's own USB-side state (unplugged/replugged from the PC, no change), and the GPIO17/RXD + GND wiring itself (continuity re-confirmed twice, both by Joseph). Root cause not found — remaining candidates are a genuine internal adapter fault (plausibly from being physically bumped mid-session) or a marginal connection internal to the adapter breakout a multimeter probe at its accessible header wouldn't catch; no spare adapter on hand to isolate further. **OFF case confirmed a different way, once the UART became unusable**: with the device docked, Intent was switched ON — this immediately dropped the MQTT connection (expected, undocumented-until-now design: Intent-on is treated as a real session-start regardless of dock state, same as the WiFi-disable logic elsewhere in this file). Confirmed via the actual Environmental Data Sheet (`action=export` query against the shared Apps Script) that two real readings landed exactly inside the Intent-on window — `2026-09-16T18:28:56Z` and `18:30:56Z`, PM2.5 1.9 and 2.0 µg/m³ — while every heartbeat before and after that window (Intent off) consistently showed `PM2.5: unavailable`. Confirms the gate works correctly in both directions on this perfboard build, independent of the debug-UART fault.
