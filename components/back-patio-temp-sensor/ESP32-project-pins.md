...
**`esp32-pins-photo.jpg` is the *underside*, so its left/right are mirrored against this table (added 2026-09-21).** **It is also not this unit's own board** (corrected 2026-09-22): the file is byte-identical to `components/air-quality-monitor/esp32_pins.jpg` and entered the repo under CARD-0012. Same board batch, so every label reading below holds for the batch — but `JCTsh-Build-Standards.md` §1.2's "verify against the actual board" rule is satisfied only at batch level here, not for this specific board. The silkscreen labels are on the bottom of the board, so a photo of them necessarily shows the reverse: pins 1–19 (3V3, EN, …) appear on the *right* in that photo, and pins 38–20 (GND, GPIO23, GPIO22, …) on the *left*. Use the photo to read what a pad is *labeled*; use this table and `ESP32pins.png` for which side it is on.
