...
**Verification requirement (added 2026-07-23, after CARD-0087):** every consumer above except GPSLogger has a way to machine-confirm the update actually stuck — Node-RED's env var via `/proc/<pid>/environ`, the read/export side via `action=version`. GPSLogger's config lives only on the phone, outside anything checkable remotely. **A URL update is not done until a live field test confirms new trackpoints actually reach the GPS Track sheet** — updating the setting alone is not sufficient, and this is not optional just because it's "only" a URL swap. CARD-0087 found the URL had silently reverted to a bare, parameter-less URL (no `action=gps`, no placeholders, no `key`) — the script correctly rejected every request with `{"status":"error","message":"unauthorized"}`, but returns that as HTTP 200, so GPSLogger itself never surfaced any error. This went undetected for 5 days simply because no hike happened in that window to exercise it — don't assume "no error reported" means "working."
