...
2.4 pb  01.07.25 Removed Array for size and for NormalScrewhead
                 and add a variable for each array element instead,
...
Show                      =              1 ; // [0:Body and lid, 1:Body only, 2:lid only]
...
// NormalScrew: Head diameter (The hole will be the size you enter)
NSHeadSizeHeadDia         = 6 ; //[1:0.1:20]
// NormalScrew: Screw head high (The hole will be the size you enter)
NSHeadSizeHeadHigh        = 2.5; //[1:0.1:10]
// Side hole distance from the outside of the body. Check the holes position on the lid
HolePos                   =            5.0 ; //[3:0.1:20]
...
if (Buy_PBTEC_a_Beer) {color("gold") translate([0,-SizeY/2,SizeZ*2+35]) rotate([90,0,90]) linear_extrude(10) text(str("Thanks, highly appreciated!"),halign = "center", size= 22,font="Liberation Sans:style=Bold");
color("gold") translate([0,-SizeY/2,SizeZ*2]) rotate([90,0,90]) linear_extrude(10) text(str("paypal.me/pbtec"),halign = "center", size= 22,font="Liberation Sans:style=Bold");}
...
module BodyCut () // Add here your body cuts
{
    ...
    // --> Parameters: diameter, vertical high, horizontal offset
        //Hole1X=[20,20,-20]; translate([SizeX/2-WallThick-0.02,Hole1X[2],Hole1X[1]]) rotate([0,90,0]) cylinder(h=WallThick+0.04,d=Hole1X[0],center = false);
        //Hole2X=[10,30,-20]; translate([-SizeX/2-0.02,Hole2X[2],Hole2X[1]]) rotate([0,90,0]) cylinder(h=WallThick+0.04,d=Hole2X[0],center = false);
        //Hole1Y=[40,25,-25]; translate([Hole1Y[2],-SizeY/2-0.02,Hole1Y[1]]) rotate([0,90,90]) cylinder(h=WallThick+0.04,d=Hole1Y[0],center = false);
        //Hole2Y=[ 5,25,-25]; translate([Hole2Y[2],SizeY/2-WallThick-0.02,Hole2Y[1]]) rotate([0,90,90]) cylinder(h=WallThick+0.04,d=Hole2Y[0],center = false);
    ...
    // --> Parameters: wide, hight, vertical high, horizontal offset
        //Window1X=[35,20,25,10]; translate([-WallThick/2+SizeX/2,Window1X[3],Window1X[2]]) cube([WallThick+0.04,Window1X[0],Window1X[1]],center=true);
        //Window2X=[35,10,10,10]; translate([WallThick/2-SizeX/2,Window2X[3],Window2X[2]]) cube([WallThick+0.04,Window2X[0],Window2X[1]],center=true);
        //Window1Y=[35,40,25,30]; translate([Window1Y[3],-WallThick/2+SizeY/2,Window1Y[2]]) cube([Window1Y[0],WallThick+0.04,Window1Y[1]],center=true);
        //Window2Y=[35,10,28,25]; translate([Window2Y[3],WallThick/2-SizeY/2,Window2Y[2]]) cube([Window2Y[0],WallThick+0.04,Window2Y[1]],center=true);
...
module Box () {
    ...
    color("green")difference(){
        union(){
            ...
            if (Show!=1){
                translate([Gap/4,-SizeY-Dist,LidThick/2+0.02]) RoundedCube([SizeX-WallThick-2*Gap,SizeY-WallThick-2*Gap,LidThick], Rounding-WallThick/2,$fn);
                translate([0,-SizeY-Dist,0]) LidAdd ();
                translate([0,-SizeY-Dist,0]) PCB_LidMount();
                translate([0,-SizeY-Dist,0]) PCB_Lid2Mount();
            } // lid
            if (Show!=2){
                for(x=[SizeX/2-HolePosCorner,-SizeX/2+HolePosCorner],y=[SizeY/2-HolePosCorner,-SizeY/2+HolePosCorner])
                {
                    if (x>0 && y>0){
                        translate([x,y,SizeZ/2-LidThick/2-Gap/2]) difference(){
                            union(){
                                rotate([0,0,0]) ThreadCylinder ();
                                if (ConnectCylinder){ rotate([0,0,-45]) translate([0,HolePos/2,0]) cube([ScrewCylinderDia-0.02,HolePos,SizeZ-LidThick-Gap],center=true);}
                            }
                            if (AddChamfer) {
                                rotate([0,0,45]) translate([-ScrewCylinderDia/2,0,SizeZ/2-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,0]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference(){
                                translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);
                                cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);
                    }}}}
                    ...
                    if (x>0 && y<0){
                            ...
                            union(){
                                rotate([0,0,0]) ThreadCylinder ();
                                if (ConnectCylinder){ rotate([0,0,225]) translate([0,HolePos/2,0]) cube([ScrewCylinderDia-0.02,HolePos,SizeZ-LidThick-Gap],center=true);}
                            }
                            if (AddChamfer) {
                                rotate([0,0,315]) translate([-ScrewCylinderDia/2,0,SizeZ/2-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,0]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference(){
                                translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);
                                cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);
                    }}}}
                    ...
                    if (x<0 && y>0){
                            ...
                            if (AddChamfer) {
                                rotate([0,0,135]) translate([-ScrewCylinderDia/2,0,SizeZ/2-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,0]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference(){
                                translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);
                                cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);
                    }}}}
                    ...
                    if (x<0 && y<0){
                            ...
                            union(){
                                rotate([0,0,0]) ThreadCylinder ();
                                if (ConnectCylinder){ rotate([0,0,135]) translate([0,HolePos/2,0]) cube([ScrewCylinderDia-0.02,HolePos,SizeZ-LidThick-Gap],center=true);}
                            }
                            if (AddChamfer) {
                                rotate([0,0,225]) translate([-ScrewCylinderDia/2,0,SizeZ/2-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,0]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference(){
                                translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);
                                cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);
                    }}}}
                ...
                if (AddXScrew) { //2 Cylinder for screw thread x
                    difference(){
                        union(){
                            translate([0,-SizeY/2+HolePos,SizeZ/2-LidThick/2-Gap/2])  ThreadCylinder ();
                            translate([0,SizeY/2-HolePos,SizeZ/2-LidThick/2-Gap/2]) ThreadCylinder ();
                            if (ConnectCylinder)
                            {
                                translate([0,SizeY/2-(HolePos-WallThick)/2-WallThick,(SizeZ-LidThick-Gap)/2]) cube([ScrewCylinderDia-0.02,HolePos-WallThick,SizeZ-LidThick-Gap],center=true);
                                translate([0,-SizeY/2+(HolePos-WallThick)/2+WallThick,(SizeZ-LidThick-Gap)/2]) cube([ScrewCylinderDia-0.02,HolePos-WallThick,SizeZ-LidThick-Gap],center=true);
                            }
                        }
                        if (AddChamfer)
                        {
                            translate([0,SizeY/2-HolePos-ScrewCylinderDia/2,SizeZ-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,90]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference(){
                            translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);}
                            translate([0,-SizeY/2+HolePos+ScrewCylinderDia/2,SizeZ-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,270]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference()
                            {
                                translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);
                                cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);
                            }
                ...
                if (AddYScrew) { //2 Cylinder for screw thread y
                    difference(){
                    union() {
                            translate([SizeX/2-HolePos,0,SizeZ/2-LidThick/2-Gap/2]) ThreadCylinder ();
                            translate([-SizeX/2+HolePos,0,SizeZ/2-LidThick/2-Gap/2]) ThreadCylinder ();
                            if (ConnectCylinder)
                            {
                                translate([SizeX/2-(HolePos-WallThick)/2-WallThick,0,(SizeZ-LidThick-Gap)/2]) cube([HolePos-WallThick,ScrewCylinderDia-0.02,SizeZ-LidThick-Gap],center=true);
                                translate([-SizeX/2+(HolePos-WallThick)/2+WallThick,0,(SizeZ-LidThick-Gap)/2]) cube([HolePos-WallThick,ScrewCylinderDia-0.02,SizeZ-LidThick-Gap],center=true);
                            }
                        }
                        if (AddChamfer)
                        {
                            translate([SizeX/2-HolePos-ScrewCylinderDia/2,0,SizeZ-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,0]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference(){
                            translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia,center = true);}
                            translate([(-SizeX/2)+HolePos+(ScrewCylinderDia)/2,0,SizeZ-LidThick-Gap-HoleDeepness]) rotate([180,-ChamferAngle,180]) translate([ScrewCylinderDia/2,0,SizeZ/2]) difference()
                            {
                               translate([-SizeZ/2,0,0]) cube([SizeZ,ScrewCylinderDia,SizeZ],center=true);
                               cylinder($fn=$fnInterior ,h=SizeZ+0.04,d=ScrewCylinderDia+0.02,center = true);
                            }
                        }
                    }
                }
        ...
        for(x=[SizeX/2-HolePosCorner,-SizeX/2+HolePosCorner],y=[SizeY/2-HolePosCorner,-SizeY/2+HolePosCorner]){translate([x,y,SizeZ-HoleDeepness-LidThick+0.02-Gap]) ThreadHoles ();} //corner thread holes
        for(x=[SizeX/2-HolePosCorner,-SizeX/2+HolePosCorner],y=[SizeY/2-HolePosCorner,-SizeY/2+HolePosCorner]){ // Corner holes in lid (countersink, normal head, only hole)
            translate([x,-SizeY-Dist-y,0]) union(){
                HolesLid (); // holes lid
        ...
        if (AddXScrew){ // holes for additional X Screws
            translate([0,-SizeY/2-Dist-HolePos,0]) HolesLid (); translate([0,-Dist+HolePos-SizeY*1.5,0])HolesLid (); // holes lid
            translate([0,-SizeY/2+HolePos,SizeZ-HoleDeepness-LidThick+0.02-Gap]) ThreadHoles (); translate([0,-SizeY/2-HolePos+SizeY,SizeZ-HoleDeepness-LidThick+0.02-Gap]) ThreadHoles (); // Thread holes for screw
            if (HoleType==1) {translate([0,-SizeY/2-HolePos-Dist,LidThick+0.04]) CounterSink (); translate([0,+HolePos-Dist-SizeY*1.5,LidThick+0.04]) CounterSink ();} // x side normal head
            if (HoleType==2) {translate([0,-SizeY/2-HolePos-Dist,LidThick+0.04]) NormalHead(); translate([0,+HolePos-Dist-SizeY*1.5,LidThick+0.04]) NormalHead();}  // x side countersink
        }
        if (AddYScrew){ // holes for additional Y Screws
            translate([SizeX/2-HolePos,-Dist-SizeY,0]) HolesLid (); translate([-SizeX/2+HolePos,-Dist-SizeY,0])   HolesLid (); // holes lid
            translate([SizeX/2-HolePos,0,SizeZ-HoleDeepness-LidThick+0.02-Gap])ThreadHoles (); translate([-SizeX/2+HolePos,0,SizeZ-HoleDeepness-LidThick+0.02-Gap])ThreadHoles (); // Thread holes for screw
            if (HoleType==1) {translate([SizeX/2-HolePos,-Dist-SizeY,LidThick+0.04])CounterSink (); translate([-SizeX/2+HolePos,-Dist-SizeY,LidThick+0.04]) CounterSink ();} // y side normal head
            if (HoleType==2) {translate([SizeX/2-HolePos,-Dist-SizeY,LidThick+0.04])NormalHead(); translate([-SizeX/2+HolePos,-Dist-SizeY,LidThick+0.04]) NormalHead();} // y side countersink
        }
        difference(){translate([0,0,SizeZ/2]) cube([SizeX+Dist*10,SizeY+Dist*2+Gap*2,SizeZ+0.04],center=true); translate([0,0,SizeZ/2]) RoundedCube([SizeX,SizeY,SizeZ], Rounding,$fn);} // Body cut outside
        translate([0,-SizeY-Dist,0]) LidCut ();
        BodyCut ();

        if(HorizontalCut){translate([SizeX/4,0,SizeZ/2]) cube([SizeX/2+0.04,SizeY+0.04,SizeZ+0.04],center=true);}
        if(VerticalCut){translate([0,SizeY/4,SizeZ/2]) cube([SizeX+0.04,SizeY/2+0.04,SizeZ+0.04],center=true);}
        if(DiagonalCut){translate([SizeX/2+0.02,-SizeY/2-0.02,SizeZ/2]) rightTriangle(SizeX+0.04, SizeY+0.04, SizeZ+0.04);}

        if(HorizontalCut){translate([SizeX/4,-SizeY-Dist,SizeZ/2]) cube([SizeX/2+0.04,SizeY+0.04,SizeZ+0.04],center=true);}
        if(VerticalCut){translate([0,-(SizeY/2+0.04)/2-SizeY/2-Dist,SizeZ/2]) cube([SizeX+0.04,SizeY/2+0.04,SizeZ+0.04],center=true);}
        if(DiagonalCut){translate([SizeX/2,-SizeY-SizeY/2-Dist,0]) rightTriangle(SizeX+0.04, SizeY+0.04, SizeZ+0.04);}
    }
}
module EasyProjectBox () {if (Position==true) {translate([0,SizeY+Dist,0]) Box();}else {Box();}}
...
module NormalHead ()    {translate([0,0,-NSHeadSizeHeadHigh]) cylinder($fn=$fnInterior ,h = NSHeadSizeHeadHigh,d=NSHeadSizeHeadDia+0.4,center=false);}
module ThreadHoles ()   {cylinder($fn=$fnInterior ,d=HoleDiaThread,h=HoleDeepness+0.04,center=false);}

module ThreadCylinder () {cylinder($fn=$fnInterior ,d=ScrewCylinderDia,h=SizeZ-LidThick-Gap,center=true);}
