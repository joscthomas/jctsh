...
6. **Review the results — the actual log, not just the console summary.**
   ...
   - For that visually-similar-but-not-byte-identical case, Immich has its own native answer: a background CLIP-embedding-based duplicate detector, surfaced as a **Duplicates** view in the web UI. It's already running (same feature `migration.md` relied on instead of a manual pre-import quality pass) and needs no new tooling — check it after a batch of newly-tagged uploads settles, as the first thing to try before reaching for a standalone tool. If it's not thorough enough, `kanban-board.md` CARD-0028 has already researched standalone options (czkawka, imagededup, fdupes/rdfind).
