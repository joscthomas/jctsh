...
3. Decided to skip a manual pre-import quality pass (blurry/duplicate/screenshot review) — relied on Immich's built-in duplicate detection (CLIP-embedding based) and an ongoing "mark favorites" habit instead. See `backlog.md` CARD-028 for optional automated post-import quality scanning tools if wanted later.
...
- ~~Spot-check the import before deleting the Takeout zips~~ — done properly (CARD-039, 2026-07-09): full re-run of `immich-go` against all retained zips found **3,433 assets that had been genuinely missing** (58 Joseph/backup, 119 Joseph/NVMe, 3,256 Robin) and uploaded them; zero errors otherwise. See CARD-039 resolution in `backlog.md` for full detail. Zips are still retained — see CARD-030 for deletion timing.
- Once satisfied with the now-more-complete import: delete zips, re-enable weekly backup cron (CARD-030)
- Optional: automated post-import quality scan for blur/near-duplicates (CARD-028)
