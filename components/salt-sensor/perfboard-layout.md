...
- **GPIO25/26 avoided** — GPIO25 (DAC1) confirmed broken for digital output in ESPHome/Arduino; GPIO26 (DAC2) avoided as a precaution for the same reinit family of issues. (Cross-reference `ESP32-project-pins.md`, though see Board Note — that doc's *position* claims are unverified against this actual board; the GPIO-number-level guidance like this one is unaffected.)
