...
- **Open question, not yet resolved:** ascent/descent (441/409 ft) look high relative to the day's actual elevation range (594-662 ft, only 68 ft of min-to-max spread per `stats.altitude_ft`). Plausibly real rolling terrain, but also plausible that raw GPS altitude noise (one sample's `accuracy_m` was 21m — vertical error is typically worse than horizontal) is still getting through the 5ft `ASCENT_DESCENT_NOISE_FT` threshold, which Planning already flagged as "one knob to retune later if it's over/under-filtering on a real hike." Needs a second real hike's data (ideally one with known/expected elevation change) to tell noise from real terrain before deciding whether to raise the threshold.
- **Hover interaction verified functionally in a real browser** (Chrome via `claude-in-chrome`, real July 30 data, not the hand-authored mockup data): dispatched real `mouseenter`/`mouseleave` DOM events against the generated page's actual hit-targets. Confirmed: hovering an elevation point shows that point's time + elevation and lights up only the elevation dot (speed dot stays hidden); hovering a speed point shows time + speed and lights up only the speed dot; `mouseleave` correctly resets to the default "Hover the chart —" state. Screenshot capture itself was flaky in this session (extension-side timeout, unrelated to the page), so this was verified via direct DOM/event inspection rather than a visual screenshot — real generated page also published as an artifact for a visual look: https://claude.ai/code/artifact/8882621b-bccb-4787-9096-5220b6a33f27

**Related:** CARD-0082 (route map + elevation profile, the visual/map-focused sibling of this stats-focused card, same underlying GPS Track data source), CARD-0104 (the Gaia-embed precedent and its "not automatable" caveat), CARD-0109 (removed the old single Elevation Range row from Data Summary, freeing that space for this card's richer stats), `components/hike-izer/fetch_hike_data.py`, `components/hike-izer/build_hike_chart.py`, `components/hike-izer/html-template.html`, `.claude/skills/hike-izer/SKILL.md`.
