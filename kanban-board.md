...
3. **External access — three real tiers, compared 2026-07-24:**
   ...
   - **Tailscale Funnel** — same tool, different command (`tailscale funnel` vs. `serve`), no port-forwarding, genuine public HTTPS access via a generated `*.ts.net` address. Lowest setup effort of the two public options, but the URL itself is an ugly, hard-to-share random `*.ts.net` address — a real downside for something meant to be sent to family after a hike.
   - **Cloudflare Tunnel + custom domain** (`jctnet.com`) — more setup (new tool/account, DNS moved to Cloudflare), but gives a real, legible custom-domain URL instead of a random `*.ts.net` string. **Cost confirmed 2026-07-24: free** for this use case — the tunnel itself has no usage limits on Cloudflare's free plan; the ~$7/user/month figures that show up in searches are for a separate, unrelated product (Zero Trust/Access identity gating), not needed for a simple public-link use case. **Leading candidate (corrected 2026-07-24)** — the one-time extra setup is worth it for a shareable custom domain instead of Tailscale Funnel's opaque address.
...
**Decided 2026-07-24: self-host on the M8**, not Google Sites. Google Sites' manual-publish friction (no programmatic API) is the deciding factor against it — self-hosting avoids that entirely and reuses infrastructure already running. Next real decision when this is picked up for build: which external-access tier (Cloudflare Tunnel is the leading candidate above, but not yet formally locked in).
