...
### CARD-0094 · [idea] [hike-izer] Switch hike-izer-web from Tailscale Funnel to Cloudflare Tunnel
**Notes:** Raised 2026-07-24, during CARD-0088's build. CARD-0088 shipped on Tailscale Funnel instead of the originally-planned Cloudflare Tunnel + `hikes.jctnet.com` subdomain, after discovering Cloudflare's free-tier onboarding doesn't support adding a subdomain as its own independent zone — it requires the full `jctnet.com` apex, which would have meant migrating the domain's nameservers to Cloudflare while it still had live Zoho email (MX + SPF records) running through it. Too risky a change for what CARD-0088 needed, so it fell back to Tailscale Funnel (already-running infra, zero risk to the domain, same pattern HA already uses) at the cost of an opaque `*.ts.net` URL instead of a legible custom domain.

**Real unblocker: CARD-0093.** Once `jctnet.com`'s DNS is cleaned up and email is genuinely disabled (CARD-0093, already decided and in Backlog), the actual risk factor that ruled out Cloudflare today — a live-email domain nameserver cutover — goes away. At that point a full-zone Cloudflare migration is a much lower-stakes action, and `hikes.jctnet.com` becomes viable again.

**Not a commitment — revisit only if worth it.** The switch is low-friction whenever it happens: the Caddy container and served content (`~/hike-izer-web-app/srv/`) don't change at all, only the exposure layer swaps (`cloudflared` container added back, Tailscale Funnel turned off). If the `*.ts.net` URL turns out to be perfectly fine in practice, there's no real reason to ever do this — don't treat CARD-0093's completion as an automatic trigger to pick this up.

**What the switch would involve when picked up:** re-add the `cloudflared` service to `components/hike-izer-web/docker-compose.yml` (removed during the Tailscale Funnel pivot — see git history around 2026-07-24), redo the `cloudflared tunnel login`/`create`/`route dns` setup against `hikes.jctnet.com` (now addable as its own zone, or as part of the full `jctnet.com` zone depending on what CARD-0093 lands on), turn off `tailscale funnel` for the port, update `credentials.local.md`/`jctsh-network.md`/this repo's docs to reflect the new URL.

**Related:** CARD-0088 (this card's parent — Hike-izer HTML hosting, in Build on Tailscale Funnel), CARD-0093 (DNS cleanup — the actual blocker/unblocker for this card).

---

### CARD-0093 · [enhancement] [personal] Clean up `jctnet.com` DNS records, disable email
**Notes:** Raised 2026-07-24, during CARD-0088's Cloudflare Tunnel setup — reviewing `jctnet.com`'s DNS in Cloudflare's onboarding scan surfaced 27 records, most of them dead cruft. Not part of CARD-0088 itself (that card doesn't touch the root domain at all) — a separate, standalone cleanup Joseph wants tracked.

**What the scan found (27 records) and what to do with each:**
- **Keep:** `CNAME www → ghs.googlehosted.com` (the real Google Sites mapping), `TXT google-site-verification=...` (keeps Google Sites/Search Console verified).
- **Remove — email is being disabled entirely (confirmed 2026-07-24: Joseph never uses `@jctnet.com` addresses):** `MX` ×3 (Zoho) and `TXT "v=spf1 include:zohomail.com ~all"` (Zoho's SPF record), `TXT "zoho-verification=..."`.
- **Remove — dead legacy cruft from a Microsoft 365/Skype-for-Business + GoDaddy-hosted-email history that predates the current (now also being removed) Zoho setup:** `CNAME autodiscover, lyncdiscover, sip, msoid` + both `SRV` records (`_sipfederationtls._tcp`, `_sip._tls`) — Microsoft federation; `CNAME e, email, mail, imap, pop, smtp, webmail, mobilemail, pda` — all point to `secureserver.net` (GoDaddy's own hosted-email infra, not Zoho); `CNAME ftp → jctnet.com` — unused FTP alias; `CNAME _domainconnect → ...gd.domaincontrol.com` — GoDaddy's "one-click setup" convenience record, not tied to anything live; `TXT "v=verifydomain MS=..."` — old Microsoft domain-verification TXT, same family as the dead Microsoft records.
- **Needs a decision, not yet made:** the root `jctnet.com` `A` records (`15.197.148.33`, `3.33.130.190`) — these aren't Google's IPs, they match GoDaddy's own domain-forwarding/parking infrastructure. This is almost certainly the root cause of the separate `/lander`-resolves-blank bug Joseph is independently investigating: the bare domain isn't serving Google Sites directly at all, GoDaddy is HTTP-redirecting it somewhere, and that redirect target is misconfigured. Fixing the forwarding target (in GoDaddy's Domain Forwarding settings, not DNS records) is a natural companion fix here, not strictly required for the DNS-cleanup part.

**End state after cleanup:** roughly 3 records total — the `www` CNAME, the Google site-verification TXT, and whatever the root `A`/forwarding setup resolves to once the `/lander` fix lands. No more email-related records at all.

**Side effect worth knowing about, not a driver of this card:** once `jctnet.com` genuinely has no live email, the single biggest risk factor against a full-domain Cloudflare nameserver migration (which CARD-0088 explicitly avoided today, falling back to Tailscale Funnel instead, specifically because live Zoho mail made that migration too risky for what CARD-0088 needed) goes away. Doesn't mean CARD-0088 should be redone — just worth knowing this reopens that path as a future option if a real reason to revisit it ever comes up.

**Related:** CARD-0088 (the card whose Cloudflare setup surfaced this — not itself blocked or affected by this cleanup), the separate (not yet carded) `/lander` blank-page bug Joseph is fixing independently.
...
**Recommendation: skip the agent frameworks entirely and use a single Claude API call.** Hike-izer's actual pipeline (`fetch_hike_data.py` → `fetch_hike_photos.py` → write Markdown/HTML) is already a deterministic sequence of existing Python scripts — nothing about *which* script to run next needs an LLM's judgment; that's exactly the kind of open-ended-exploration need the Agent SDK / Managed Agents exist for, and it isn't present here. The only step that genuinely needs Claude is turning the structured hike JSON into readable narrative prose. So the headless path is: a small Python orchestrator (webhook receiver for the Tasker POST) runs the existing fetch scripts directly via subprocess exactly as SKILL.md's steps already specify, then makes **one** Claude Messages API call (structured hike JSON + the narrative-writing instructions from SKILL.md as the prompt) to get the Markdown/HTML text back, then writes the files and pushes them to wherever CARD-0088 ends up hosting them. This is the "Classification, summarization, extraction" tier in Anthropic's own tiering guidance — reach for an agent only when the task needs genuine open-ended, model-driven exploration, which this doesn't once the data-fetching is already a fixed script sequence.

**Where this orchestrator lives — ties to CARD-0088.** The Tasker HTTP POST needs a publicly reachable receiver, and CARD-0088 is already standing up exactly that kind of reachable endpoint on the M8 (Cloudflare Tunnel, static-file container). The same M8 host is the natural home for this small orchestrator script too, rather than inventing a second piece of hosted infrastructure — worth building CARD-0086 after CARD-0088's hosting is in place, not before.

**Logging/MQTT visibility becomes possible here, not before (noted 2026-07-24 during CARD-0088 build).** CARD-0088 explicitly skips a per-publish MQTT log line for its `scp`-based deploy step, because that step runs on Joseph's Windows machine, which has no MQTT-publish capability and adding one would mean a new pip dependency for a one-off script. Once this card's orchestrator exists and lives on the M8 (a Linux host with local broker access, same as every other JCTsh MQTT publisher), that constraint goes away — the orchestrator is the natural place to add real dashboard visibility (e.g. `jctsh/hike-izer/publish/log` on a successful auto-generated summary, `Alert`-category on a failure) using the same pattern already established for the backup script and scheduled-reboot notifications. Worth building in from the start here rather than bolting on later.

**Related:** CARD-0073 (Hike-izer v1, Done), CARD-0074 (superseded), CARD-0083 (sibling trigger-mechanism need, opposite end of the hike), CARD-0007 (Hiking Observations pipeline — the phrase-match fallback and the Tasker HTTP-POST pattern the leading candidate copies), CARD-0088 (hosting — likely home for this card's webhook-receiver/orchestrator), `components/hiking-monitor/gps-pipeline.md` (GPSLogger app details), `components/hike-izer/fetch_hike_data.py`, `.claude/skills/hike-izer/SKILL.md` (the narrative-writing instructions this card's orchestrator would call Claude with).
