...
- **Not built:** item 2 (Ubuntu Pro/ESM opt-in) is a one-time preference decision, not something the recurring check monitors.
...
**Remaining, deliberately not done today:** the 6 Docker/containerd packages (held for their own deliberate pass — a `docker-ce` update restarts the daemon, touching every running container), the reboot needed to clear `libc6`'s `reboot-required` flag, and the Secure Boot firmware update (CVE-2024-7344) — all still correctly showing as an active `Alert` on the dashboard.
