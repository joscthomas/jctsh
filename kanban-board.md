...
**Open questions for build time, not yet decided:**
1. Staged-file naming convention (e.g. `gaia_embed.html`, `birdnet_export.csv`) that step 2's script looks for.
2. Whether step 2 runs as a literal separate script (`generate_step2.py`) or the existing `generation.py` gains a mode flag.
3. Whether `hike_data.json` and `place_context`'s output need to be persisted into `SRV_DIR` (not `/tmp`, which won't survive a container restart or a step-2 trigger hours/days later) so step 2 can reuse step 1's work rather than re-querying the Apps Script or re-running place-context enrichment from scratch.
4. Whether CARD-0107's deliberate decoupling of photo captions from the narrative is worth revisiting — that decoupling existed specifically because photos might not be ready when narrative ran; step 2 removes that timing constraint, so the narrative could reference real captions/`sign_text` directly if wanted.
5. CARD-0080 (BirdNET) isn't built yet — step 2's design should leave room for it to slot in later, not require it now.
6. **Confirmed bug, folded in 2026-07-29 07:29 MST, found while reviewing the July 29 narrative:** `place_context.py`'s `named_features()` only queries a 250m radius around the hike's **first GPS point** — not sampled anywhere else along the route. For a neighborhood loop that starts from the same spot every day, that fixed anchor returns nearly the same "nearby" school/park regardless of which direction that day's hike actually went. Confirmed live: the July 29 narrative gave Ottawa Hills High School a full paragraph (founding year, mascot history) even though Joseph actually walked toward Grand Rapids Christian High School that day — Ottawa Hills was near a *different* day's hike from the same starting point, mentioned only in passing here. Joseph's own diagnosis: photos (real GPS-tagged capture points along the actual route, once step 2's photo re-fetch lands) would ground this correctly where the first-point anchor can't. Fix approach not yet designed — likely sampling named features along the whole GPS track (or its bounding box) rather than one point, with photo EXIF locations as a stronger secondary signal once available.
...
1. **Bug, root cause confirmed 2026-07-29 05:44 MST — Immich upload-timing race, not a code defect.** This hike happened in East Grand Rapids, MI (offset handling correctly used `-04:00`, confirming CARD-0086's non-Arizona-hardcoded logic held up on a real out-of-state trip). Checked directly via SSH into the M8 and the orchestrator's own logs plus a live Immich `search/metadata` query:
   ...
   - **Spun off to CARD-0112 (Planning)** — the real fix isn't a targeted retry but a broader two-step generation redesign (automatic data-only publish, then a manually-triggered enrichment + narrative pass once photos/Gaia/bird data are actually staged). Closing this item here as diagnosed; tracked forward in CARD-0112.
