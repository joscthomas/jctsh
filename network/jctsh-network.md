...
| back-patio-temp-sensor ESP32 | 192.168.1.188 | back-patio-temp-sensor.local | 04-B2-47-97-DF-44 | ESPHome (CARD-0219); DHCP-reserved 2026-09-21 (Joseph) and **verified by a real reboot, not just by the router accepting the entry** — device restarted via its MQTT restart button, came back on the same IP, and reconnected to MQTT. Same board batch as hiking-monitor (MAC one serial apart, `...DF-2C` vs `...DF-44`). |
| air-quality-monitor ESP32 | 192.168.1.134 | air-quality-monitor.local | 28-05-A5-E2-80-48 | ESPHome (CARD-0012); DHCP-reserved 2026-09-25 (Joseph). MAC read from the ARP table while the device was on the LAN, not from the router's reservation page. WiFi is only up briefly while docked (Intent off, power connected, battery >= 3.5 V), so it is not always reachable for OTA -- see CARD-0012. |
...
| Hike-izer backstop check (missed-hike recovery) | Daily, 5:00 AM | M8 | `kanban-board.md` CARD-0121 |
