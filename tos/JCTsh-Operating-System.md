...
- **A protocol change lands mid-day — notify the live sessions, don't wait for their next startup (CARD-0325, 2026-09-22, Joseph asking "how do the other sessions know about the change in protocol?").** This document is read at session start, so a rule added to it reaches *future* sessions only; a session already running never re-reads it and will keep following the old protocol indefinitely — the exact sessions most likely to be affected, since a rule usually gets written because live sessions are colliding right now. **So: whenever a change to this document or to a `tos/` process doc changes how other sessions must behave, immediately `ListAgents` and `SendMessage` each live peer** with what changed, where to re-read it, and anything specific to their in-flight work. Not a courtesy — without it the change isn't actually in force until every session happens to restart. Done live at this rule's own creation: three peers (`ops`, `jctsh-47`, `porch/patio temp sensors`) notified of the Board Commit Rule within minutes of it landing. A change that only affects *future* work (a new card convention, a doc reorganization nothing acts on) doesn't need this — judge by whether a session could act wrongly today.

- **Card ID allocation — allocate late, let the push serialize it (CARD-0325, 2026-09-22).** **What a collision actually costs, stated plainly because it was first written down overstated as "real data loss":** nothing is lost. Every session shares one working tree and every write re-reads the file at apply time, so the loser of an ID race still has its card text on disk — what you get is two cards sharing one number, meaning ambiguous cross-references, a duplicate in the `/kanban` parser, and a renumber to clean up. Worth closing because ambiguity in card references is corrosive to a board that *is* the project's record, not because work would disappear. The `<!-- next-card-id -->` marker is a convenience, not a lock: three sessions claimed CARD-0322 through CARD-0328 within ~45 minutes on 2026-09-22. Four mechanical steps make that safe without any coordination:
