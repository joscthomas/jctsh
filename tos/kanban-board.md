...
**Superseded target, 2026-09-17 — a second maintenance-check finding (PR #88) landed for v3.2.2 before v3.2.1 was ever applied.** Checked the new release's notes too rather than assuming: v3.2.2 (2026-09-15) is a single-issue bug-fix patch (face-reassignment now correctly skips other users' faces during reassignment) — no breaking changes, no database migration, same low-risk profile as v3.2.1. Folded into this card rather than opening a duplicate; target bumped straight to v3.2.2 since applying it supersedes v3.2.1 entirely.
