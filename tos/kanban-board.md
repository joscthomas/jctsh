...
**Same session, continued — checks 5-7 mostly closed out; the debug UART adapter developed a real fault partway through.** Check 5 (Intent switch): confirmed ON correctly triggers `DUTY CYCLE: switching to Measurement mode` via COM9. Right after, the debug adapter's RXD line went permanently dark — systematically isolated (stdout buffering, stale process handle, USB re-enumeration, the adapter's own USB-side state, and the GPIO17/GND wiring itself, the last one re-confirmed twice by Joseph) down to either a genuine fault in the adapter module or a connection internal to it, not this project's own wiring. **Parked** — Intent-OFF's silent-duty-cycle confirmation is the one open item, picks up with a spare adapter swap next time. Pivoted to the docked/MQTT dashboard path for the remaining checks, since it doesn't depend on the debug UART: check 6 (battery divider sanity) confirmed clean (4.02V multimeter vs. 4.04V logged); check 7 (full boot) confirmed via a clean dock connect, buffered-replay, and a sane 5-min heartbeat. Full detail in `components/air-quality-monitor/perfboard-layout.md`'s own check 5-7 entries. **Step 9's checklist is functionally complete** except check 5's parked half. Also noted in passing, not investigated: the Pi's raw `/mnt/jctsh-logs/jctsh.log` lagged several minutes behind what the rendered dashboard showed for this device's heartbeat — worth a look sometime.
