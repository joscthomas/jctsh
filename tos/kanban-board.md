...
**Concrete first-pass mechanics for the hike-izer pilot, worked out 2026-09-17 — deliberately using only tools that already exist, nothing to build:**
- **Starting/resuming:** nothing special on day one. Next time hike-izer work comes up, work in that session as normal. The one discipline change: **resume** that conversation next time (`claude --resume`, picked from the list, or `--continue` if it's the most recent) instead of always starting a fresh `claude` session cold. This *is* the persistence mechanism — session resumption across time, not a background daemon. No formal naming convention yet; `--resume`'s own picker shows recent session content, which should be enough to recognize "that's the hike-izer one" by eye during the pilot. Revisit if that gets ambiguous once more domain sessions exist.
- **Gathering info from another domain mid-problem:** default to reading that domain's own files directly (its `CLAUDE.md`, its code) — always available, no dependency on anything else being open, exactly what a fresh session would do anyway. `ListAgents`/`SendMessage` (this environment's built-in way for separate local Claude Code sessions to find and message each other) is a bonus shortcut *only* when another domain's session happens to be concurrently open at that exact moment — never a hard dependency, since there's no guarantee it's alive when needed.
- **Distributing findings after solving:** no new mechanism — this is the existing `Related:` cross-reference convention, applied consistently. The finding gets written into whichever domain's own docs it actually belongs to (not just hike-izer's, if the fix lived elsewhere), and the kanban card gets tagged so anyone scanning by component later finds it. Persistence only changes *who* does the writing (the session that solved it, not a fresh one reconstructing what happened) — the destination and format are unchanged from today's practice.
...
**Done when:** not a single deliverable — this is an ongoing practice to adopt, not a build. Revisit once the hike-izer pilot has actually been used a few times, to decide what (if anything) needs to become a formal convention (and get promoted into `JCTsh-Operating-System.md`) versus staying ad hoc.

**Related:** CARD-0283 (the concurrent-editing convention change from the same discussion thread), CARD-0251 (the Auto verify/Watch for markers precedent this follows for how a proven ad hoc practice gets promoted into real TOS documentation), `components/hike-izer*/CLAUDE.md` (the pilot cluster's docs), `tos/kanban-board.md`, `tos/JCTsh-Operating-System.md` (the eventual home for this, once proven).
