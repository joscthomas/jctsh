...
**Testing.md checklist not fully run through the Log Dashboard/HA UI this pass** — this session's `/status`/dashboard access is blocked on Basic Auth credentials (see session-start note). Sensor Validation (temperature) and the Heartbeat/Watchdog checks are covered by the log evidence above; the Log Dashboard check, humidity/pressure/illuminance plausibility, and the Lux Sensor cover/uncover test still want an actual look at HA or the dashboard to close out `testing.md` fully.
...
- **MQTT discovery collision** (found above) — pick a fix option and apply it, then re-verify `testing.md`'s Sensor Validation step actually shows values in HA.
