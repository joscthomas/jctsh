...
**Root cause:** `_classify_hike()` computes one median point-to-point speed across the *entire* session and rejects if it's below `WALKING_SPEED_MIN_MPS`. A long rest stop in the middle of an otherwise-real hike drags that whole-session median down below the threshold even when a real walking-pace stretch clearly happened at both ends — the classifier can't currently distinguish "mostly stationary the whole time" (a real rejection case: GPS drift while parked or camping) from "walked, rested a long time, walked back" (a real hike this check wrongly rejects).

**Not a new problem class for this file** — `_truncate_trailing_fast_activity()` (CARD-0101) and the accuracy/speed-windowing fixes (CARD-0140) both already exist because a single whole-session median is too coarse a signal for real-world hike shapes. This is the same class of gap, opposite direction: CARD-0101 handles a *trailing* fast segment corrupting classification; this is a *rest stop* corrupting it.

**Proposed fix direction (not yet finalized — needs Build-time tuning against real data, same discipline as CARD-0101/CARD-0140):** when the whole-session median trips the "too slow" rejection, don't reject outright — check whether the session nonetheless contains a real, sufficient stretch of walking-pace movement using the already-existing `_moving_stopped_time_min()` (currently only used for the post-classification pace stats, not classification itself). If `moving_time_min` clears some minimum floor (candidate: same ballpark as `MIN_HIKE_DURATION_MIN`) *and* the session's net displacement/spread from its own centroid clears a minimum real-distance floor (to keep rejecting GPS-drift-while-parked, which stays within a few meters), reclassify as `is_hike: true` — the rest period just becomes part of the hike's own `stopped_time_min`, exactly like a normal mid-hike break already is for a session that never dipped below the whole-session median floor in the first place.

**Explicitly needs real calibration, not a guessed threshold shipped blind:** the "minimum real-distance floor" in particular has to be picked to clear real drift noise (this file's own accuracy-filtering work, CARD-0140, found meaningful drift on the order of tens of meters, not hundreds) while not being so high it misses a legitimately short there-and-back hike.

**Built, deployed, and verified live, 2026-09-08.** Implemented in `_classify_hike()` (`components/hike-izer/fetch_hike_data.py`): when the whole-session median trips the "too slow" branch, builds a full `_hike_point_series()` for the candidate points (same accuracy-filtering/windowed-speed machinery CARD-0140 already uses), computes `moving_time_min` via the existing `_moving_stopped_time_min()`, and computes `max_spread_m` (max haversine distance from the session's first point to any other point). Rescues (treats as `is_hike: true`) only if `moving_time_min >= RESCUE_MIN_MOVING_MIN` (10.0, same bar as `MIN_HIKE_DURATION_MIN`) **and** `max_spread_m >= RESCUE_MIN_SPREAD_M` (150.0m, well clear of CARD-0140's own ~20m real-drift-while-parked finding) — both conditions required so ordinary GPS jitter while genuinely stationary can't accidentally clear the bar. Diagnostics (`rescue_moving_time_min`/`rescue_max_spread_m`/`rescued_by_moving_stretch`) added to the returned `details` dict, present only when this branch actually ran. The "too fast" (vehicle) branch is completely untouched — no rescue logic there.

Verified three ways before and after deploying:
1. **Real 2026-09-08 data** (already fetched for the anomaly review): now correctly rescues — `is_hike: True`, 42.8 min moving, 980m max spread.
2. **Synthetic realistic parked-drift session** (bounded ~15m random-walk jitter around one point, 60 min, matching CARD-0140's own real-drift-magnitude finding): correctly stays rejected — 0.0 min moving, 17m spread, well under both rescue floors.
3. **Synthetic all-vehicle-speed session** (~15 m/s steady, 20 min): unaffected, still rejects on the pre-existing "too fast" branch with an unchanged message.
...
- `README.md` gets a new webhook-contract entry documenting `POST /webhook/step2?key=<WEBHOOK_SECRET>`.

**Tasker side — revised 2026-09-05 (Joseph's call), first use of a new general-purpose pattern instead of a dedicated single-tap icon.** Rather than one Task + one home-screen icon per new no-input backend action (which is how `Log Idea`/`Log Observation` were each built), this becomes the **first entry in a new `JCTsh Menu` Tasker task**: one home-screen icon opens a Tasker **Menu** action listing available fire-and-forget actions by name, and the selection branches (`If %menu_selection ~ "Run Step 2"`) into an `HTTP Post` block for that action. Adding a future no-input action later means adding one more menu entry + branch inside this same task, not a new Task and a new icon each time. **`Log Idea` and `Log Observation` explicitly stay as their own dedicated one-tap icons, unchanged** — Joseph's call: a single unconditional tap is worth keeping for those two rather than folding them into a menu that costs an extra tap (open menu → pick item). Build guide moved to `components/jctsh-menu/tasker-setup.md` (CARD-0241) rather than living here — Menu action → per-entry HTTP Post branches, "Run Step 2" as its one entry for now, added to the home screen via the Tasks tab's **Add to Launcher** (the route that actually works on this Tasker version, per CARD-0173's own finding — not the Widgets flow).
...
**Done when:** (1) the actual reboot trigger is identified via a real live capture, not just ruled-out candidates, and fixed or confirmed benign; (2) the replay path tracks delivery per-record (e.g. QoS 1 with a real broker ack, removing just that one line once confirmed) instead of all-or-nothing, so a mid-replay interruption -- from this bug or any future one -- can't cost real data; (3) verified live against a real hike with a large buffered-reading count, confirming no reboot loop and no data shortfall. **Still not met** — three recurrences now confirmed (2026-08-29, 2026-09-03, 2026-09-08), all adding evidence and confirming this isn't a one-off, but none resolving anything: no live UART capture has happened yet (still blocked on physically running CARD-0205's debug setup during a real occurrence, or forcing a deliberate bench reproduction), and the replay-path robustness fix (per-record delivery tracking) hasn't been built.

**Watch for:** hiking-monitor's durable log showing a `"Reboot request from mqtt"` (or any blank/empty) field-mode reset-reason line from a hike **after 2026-09-08** — a fourth recurrence beyond the three already logged above. Three occurrences in ~10 days suggests this happens often enough that the next one is likely soon; if it shows up, log it the same way as the prior three (exact reset-reason text, real event timestamps via "Display refreshed" lines, whether it's the 2026-08-29 post-replay-burst shape or the 2026-09-03/09-08 spread-through-the-hike shape) — and flag that CARD-0205's debug UART setup should be running on the *next* hike after that, since a caught-live occurrence is the only thing that actually advances this card past ruled-out candidates.
...
- **All of Step 7's remaining items are now verified** except one still-open thread: `VOUT` read 3.18V (not the expected clean 3.3V) on battery alone right after the switch rewiring — traced to the battery needing a real charge, not a wiring fault (the "3.9V" reading that prompted the check was taken mid-charge, which reads artificially high — see Step 8's bench-sweep caution above). Battery back on the charger; final `VOUT`-on-battery-alone recheck pending a real charge, not blocking on anything else.
