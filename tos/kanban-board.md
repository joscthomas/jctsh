...
**Remaining scope, not yet swept:** `automatic-garage-door-opener-closer`, `bedside-clock`, `equip-shelf`, `front-porch-temp-sensor`, `garage-presence`, `garage-radar`, `outdoor-presence-detection`, `p-w-firefly`, `photo-quality-review`'s own content (only its root-table entry was added, not its own doc's accuracy), `remote-temp-sensor-01`, `salt-sensor`'s own content, `traveling`, `van-sensors`, `weather-station`'s own content, `core/homeassistant`, `core/logging`, `core/maintenance`, `core/mqtt`, `hosts/pi1`'s own content. Card stays in Build.
