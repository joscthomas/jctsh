...
### CARD-0235 · [idea] [hike-izer] BirdNET Live tracks continuous GPS during every hike — evaluate turning it off, but check the species-ID accuracy tradeoff first
**Status:** Planning
...
**Genuinely unresolved, not yet checked:**
1. Whether **BirdNET Live** (the mobile app actually used here) wires Survey Mode's GPS into that same species-range-filtering model, or handles/uses location differently than the research above (which centers on BirdNET-Analyzer/the general model family, not confirmed against the Live app's own settings/behavior).
2. Whether that filtering — if it applies here — needs the *continuous* GPS polling described above, or would work just as well from a one-time starting location/region set once at Survey Setup. Those have very different battery costs; "keep a coarse one-time location, disable continuous background updates" could get most of the battery savings with none of the accuracy risk, if the app supports it.
3. Whether the accuracy difference (if real for this app) is even large enough to matter for this project's actual use — casual hike bird-ID, not a scientific survey needing maximum precision.

**Done when:** BirdNET Live's own settings/documentation (or direct testing) confirm how Survey Mode's location setting is actually used by the app, and a real decision is made — disable GPS entirely, switch to a one-time/coarse location only, or keep continuous GPS with the reasoning for why the accuracy tradeoff is worth it — not just theorized here.

**Related:** `components/hike-izer-orchestrator/birdnet-pipeline.md` Section 4 (CARD-0182 — the Live→Survey Mode decision this follows on from), CARD-0229 (the broader BirdNET data-architecture-and-MQTT review this is a narrower, battery-focused sibling to — kept separate since this is a concrete actionable question, not part of that card's architecture-review scope), CARD-0227 (the image-attachment feature this PR tested, which is how the screenshot made it into the original finding).
...
**Done when:** the four original review questions all have real, code-verified answers — **met**. The implementation plan above is written and decided — **met**. Actual code not yet written; that's the next step once Joseph says go.
