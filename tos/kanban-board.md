...
**Open question, raised 2026-08-24 (Joseph):** how does hiking direction relative to the sun affect the LTR-390's UV reading, given the sensor sits on the front of the hiker's body? Joseph's own observation: hiking into the sun reads noticeably higher than hiking away from it.

**Answered, 2026-08-24 (Claude's explanation, not yet independently verified against real data) — this is a real, physically-grounded effect, not noise:**
- UV Index is conventionally defined on a **horizontal, upward-facing plane** — a sensor pointed straight at the sky. The LTR-390 here is mounted vertically on the front of the body, so it's actually measuring irradiance on a **vertical, forward-facing plane**, a genuinely different quantity that depends heavily on the angle between the sun's position and the direction the hiker is facing.
- **Hiking into the sun:** the direct beam lands close to perpendicular on the sensor's face, and a cosine-response photodiode couples most of that direct-beam energy into the reading — reads high, and can actually exceed what a horizontal sensor shows at moderate sun angles (a vertical surface peaks at low-to-moderate solar elevation — morning/late afternoon, sun near eye-level — not at solar noon, unlike a horizontal sensor).
- **Hiking away from the sun:** the direct beam hits the hiker's *back* instead. The front-mounted sensor sees essentially none of the direct-beam component — it's on the sun's "shadow side" — leaving only diffuse skylight from the patch of sky ahead plus whatever reflects off the terrain in front. That's why the drop is so pronounced: the dominant term is nearly absent, not just partially attenuated.
- **Consequence for this card's own idea:** the raw UV Index number isn't purely a location/time property — it's confounded by which way the hiker is currently facing relative to the sun. A "green" reading while walking away from the sun doesn't mean real UV exposure risk is low — arms, the top of the head, and the back are still getting close to the true ambient UV regardless of which way the sensor points. Recoloring the *raw* number by risk level as originally proposed could genuinely mislead rather than clarify.
- **A possible path forward, not yet designed:** hike-izer's pipeline already computes both `travel_bearing_deg` (GPS heading) and `sun_azimuth_deg`/`sun_elevation_deg` per point (CARD-0085/CARD-0110) — the raw ingredients for a direction-corrected UV estimate already exist in the data, if that's worth pursuing at Planning.
