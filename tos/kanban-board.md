...
**Desktop centering verified with real geometry, live:** `getBoundingClientRect()` on the real page shows Route Map's card at `y:614, h:394` inside a combined-column height of `h:595` (`y:514`–`1109`) — 100px of space above the map, 101px below, essentially exact centering. **Mobile scoping verified by construction, not by an actual resized viewport** — `resize_window` reported success but the tab's real `innerWidth` never changed in this session's browser environment (a tool limitation, not a page bug), so this rests on the `align-items` change sharing the identical, already-proven-correct media-query block as the 2-column layout itself, not a separately re-tested mobile screenshot.
