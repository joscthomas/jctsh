...
**Procedure documented, 2026-09-26 (option 5 -- done; the firmware options remain open).** Joseph confirmed the intended routine: docked = Power Switch ON + Intent OFF (uploads and syncs the clock); starting a hike = unplug and flip Intent ON, **never touch the Power Switch** (it is a forced-restart / storage tool, not part of the cycle). `components/air-quality-monitor/operations.md` now says so: a new "The Clock (no RTC)" section (time comes only from a docked SNTP sync and is lost on any power-cycle; with no valid clock every reading is skipped -- the 9/26 incident is cited), "Normally leave the Power Switch ON", and rewritten "Before outdoor use" and "Storage" workflows (confirm the `Air quality monitor online` line before leaving; after a power-cycle or storage, dock with Intent OFF and wait for it to connect first). The doc previously never mentioned the clock, and its old step 1 ("confirm the Power Switch is on") read as if switching it on right before leaving was fine. This does not protect against an accidental reset or brownout in the field -- options 1-4 still do that.

**Option 1 built and flashed, 2026-09-26 (Joseph: "do this: keeping readings, tagged by uptime, instead of discarding them"; "flash it").** With no valid clock the AQM now stores the reading instead of skipping it: `"ts":null,"uptime_s":<seconds since boot>,"boot":"<per-boot id>"`, always buffered (never published live, so an unresolved time can never reach the Sheet). At the dock replay, a tagged line from the **same boot** with a now-valid clock is rewritten to a real ISO `ts` = now - (uptime_now - uptime_at_reading), exact to about a second; a line from an **earlier boot** is forwarded still tagged. Stale-sensor and NaN skips are unchanged. Node-RED router `env-data-route-skip-reset` carve-out: a `/data` message with `ts:null` **and** `uptime_s` is logged on the component's `/log` as `Unresolved-time reading kept in log: {...}` (System, first 400 chars) instead of being Alert-dropped; `ts:null` without `uptime_s` and readings with no `ts` still Alert-drop. Verified: compiles (config_hash 0x6ec05797), OTA-flashed to 192.168.1.134, came back online (`Reboot request from esphome.ota`); router 7 mock checks pass. **Router deployed live 2026-09-26 11:4x** (Joseph authorised reading the Node-RED admin password from `credentials.local.md` for this one deploy; `deploy_flow.py` can't be used as-is because the repo tab id differs from the live one, so only the live router node's function was replaced, by name, with the POST queue confirmed empty; verified live function == repo, then a tagged test reading on a throwaway `aqm-router-test` topic produced the `Unresolved-time reading kept in log` System line and no Sheet write). **Not yet verified:** the cold-boot bench test (power-cycle with no Wi-Fi, Intent ON for several cycles, dock, confirm readings are stored and the same-boot replay produces correct timestamps). **What it does not fix:** a run whose boot is not the boot that docks (e.g. 9/26: power-cycled at the dock after the run) keeps its readings but their times stay unresolved -- they sit in the log tagged by uptime, and need option 6 (anchor to the hike window) or option 3/4 to become real timestamps. Options 2 (this reconcile, same-boot case) and 1 are done; 3, 4 and 6 remain open. Correction to the 9/26 post-mortem: the 11:06 upload after the bench session was fine -- 11 readings (17:44-18:04Z) reached the Sheet because the boot had synced its clock at the 09:16 dock; only a boot with no sync loses data.

**First cold-boot bench test, 2026-09-26 12:12 -- storage worked, reconcile did not (bug found and fixed, fix compiled, not yet flashed).** Undocked, power-cycled, Intent ON ~22 min, re-docked Intent OFF without touching the Power Switch. The AQM buffered the readings uptime-tagged (uptime_s 162, 282, ... 1242+, one per 2 min, no gaps, one boot id) and replayed them at 12:12:44; the live router logged each as `Unresolved-time reading kept in log` and none reached the Sheet (correct fallback). But they were **not resolved to real timestamps**: the replay script fires on MQTT-connected, ~2 s after Wi-Fi comes up, before SNTP has synced, so the same-boot reconcile saw an invalid clock and forwarded everything still tagged. Fix (`attempt_aqm_replay`): when this boot has tagged readings (`boot_id` != 0) and the clock is not yet valid, `wait_until` the clock is valid, up to 60 s, then replay regardless. Compiled (config_hash 0x680f38f6). Flashing reboots the AQM, which changes the boot id -- so the test must be re-run from a fresh cold boot after the flash; the 9/26 12:12 readings are preserved only in the Pi's log (each line carries uptime_s; boot time ~11:50 from the LWT/undock at 11:50:25). Also unexplained: the replay announced 23 lines but the log showed 10 tagged readings + 3 wifi_attempt_start events (the log server collapses/holds lines; not chased).
---
...
- **Decision (d), 2026-09-25 20:23 MST -- Joseph: "no, re-check each reading"** (rejecting "accept the edge-only behavior"): the alert switches are reconciled against the salt level on **every** reading. Implemented in `salt-sensor.flow.json`'s `fn_threshold` -- each reading now sends `switch.salt_critical_alert` = (percent < 15) and `switch.salt_low_alert` = (15 <= percent <= 33) unconditionally; `criticalSent`/`warningSent` are still maintained (the reset path reads them); log lines stay transition-based plus one `[System] ... still active ... re-asserted` line per reading while an alert is active. Thresholds untouched. **Tested against mocks on the Pi's Node v22 (computation only, nothing live): 11 cases -- first critical, critical repeated with the flag already set (the case that hid this bug), warning<->critical, warning->ok, ok idle, both boundaries (15% = warning, 14% = critical), test-mode gate, test-mode reading -- all as designed.** Side effect Joseph chose: a switch found off in the alert zone is turned back on at the next reading (every 12 h and on every device reboot), which re-triggers the Google Home routine; an already-on switch is a no-op in HA.
- **Not yet deployed:** the change exists only in the repo file (uncommitted, per `core/node-red/README.md` -- commit only after deploying). Deploy is Joseph's, from his own terminal, because it needs the Node-RED admin password: `python core/node-red/deploy_flow.py components/salt-sensor/salt-sensor.flow.json tab_saltlevel`. Replacing the tab restarts that flow only; nothing is re-asserted until the next reading (a salt-sensor restart or the 12 h tick).
- **Deployed and verified live, 2026-09-25 20:28 MST (Joseph ran `deploy_flow.py`, entering the admin password himself):** the script reported `change: ['fn_threshold']` only and `Verified: all 18 nodes present, every function node's code matches the repo`; flow file committed after (`8e961e7`). Live proof the new code runs: salt-sensor restarted via its MQTT button at 20:27:04; on the boot reading (`Salt: 4% (42.1 cm)`) Node-RED logged `CRITICAL still active — salt at 4%. Alert switch re-asserted.` -- a line only the new code can produce, also seen on Joseph's dashboard. HA afterwards: `switch.salt_critical_alert` **on since 17:04:57** (unchanged -- re-asserting an already-on switch is a no-op, as designed), `switch.salt_low_alert` off, `input_number.salt_level_percent` 4.0 (updated 20:27:11). Done-when (a), (b), (d) met; (e) done.
- **Not tested live, on purpose:** the *repair* path -- turning the switch off by hand and seeing the next reading turn it back on. It was covered by mock case 3 and would re-fire the Google Home routine for a test, so it was left for Joseph to ask for.
- **(c) answered, 2026-09-25 20:32 MST -- and it corrects an assumption this card and this session made.** Joseph: "nothing played" when the switch turned on at 17:04:57. **That is what the design predicts, not a residual bug:** CARD-0261 (`components/salt-sensor/card-archive.md`) states the alert switches are "used purely for Google Home app/voice visibility and manual control... No [SmartThings] Routine reacts to any of them... bookkeeping/visibility only", and live HA agrees -- `search/related` on `switch.salt_critical_alert` returns **no automations, no scripts, no scenes**; the switch is exposed to `cloud.google_assistant` (also `cloud.alexa`, `conversation`), which is state/voice-query visibility, not an announcement. Earlier text here and in chat that said a "Google Home critical routine" would fire was an unsupported assumption, not a documented fact; whether Joseph has a personal Google Home routine keyed to the switch is unknown to the repo (he reports none played). So what the fixed switch delivers today is: `on` in the Google Home app, an answer to a voice query, and the Node-RED `Alert`-category line on the dashboard -- **not** an audible announcement or a push.
- **Resolution, 2026-09-25 20:32 MST:** Done-when (a) helper shape matches Salt Low Alert; (b) `switch.salt_critical_alert` is on and stays on with salt critical (on since 17:04:57, re-read at 20:28); (c) answered above -- no routine reacts, by design; (d) decided by Joseph ("re-check each reading") and deployed; (e) deployed and verified live (`8e961e7`, re-assert log line seen). **Reflection:** the lasting lessons are on this card -- Node-RED's HTTP 200 does not prove a HA switch stayed on (`fn_ha_log` only flags non-200), and CARD-0280's end-to-end test read that log line as success; verify a state change by re-reading the entity, with a *fresh* `hass` object each read.
- **Follow-on Joseph may want, not opened (no request yet):** an audible or push critical-salt alert. Nothing provides one today. Options if wanted: an HA automation on `switch.salt_critical_alert` -> `on` that announces on a Google speaker (`media_player.*` is already used in `automations.yaml`) or sends a companion-app push; or a Google Home routine keyed to the switch. Its own card, with an interview first.
...
- **Cadence, verified live 2026-09-25 16:54 MST:** salt-sensor heartbeat at 16:51:20 (`uptime 0h 29m`, 30 min after its 16:21:18 restart) and garage-radar at 16:54:06 (`uptime 0h 30m`, 30 min after its ~16:24:06 OTA reboot) -- the 30-minute interval continues after the boot-time heartbeat on both.
- **HA consumers, verified live 2026-09-25 16:41 MST (Joseph walked in front of the radar):** presence went ON at 16:41:24.7 on MQTT and in HA the same second, moving/still targets and the distance stream updated, presence cleared 16:42:04 (30 s `delayed_off` after still-target OFF at 16:41:50); "Restart timer on activity" ran and started the 900 s timer. Salt: `input_number.salt_level_percent` / `sensor.softener_salt_level` updated 16:10:33 right after the flash reboot. Read-only, via HA's in-page state in Joseph's signed-in Chrome.
- **Radar deliberate restart, done 2026-09-25 17:00 MST -- by power cycle, because garage-radar has no restart button (only salt-sensor does; a first scripted attempt aborted before pressing anything, so nothing was touched).** Joseph unplugged/replugged the USB-C: online + MQTT connected 16:59:11, boot heartbeat 17:00:36 (85 s later; `uptime 0h 1m`, `rssi -47`, `presence OFF`). A cold boot rather than the OTA warm reboot, so it is the stronger test.
- **Last gap closed, 2026-09-25 17:02 MST -- the watchdog-alert check, done from the Pi's durable log instead of the dashboard (no `DASHBOARD_PASS` needed; same source the dashboard reads).** `awk` over `/mnt/jctsh-logs/jctsh.log` from 15:50 to 17:02 for Alert-category lines, `watchdog` as component, and `silent`/`not reported` text: **no watchdog line and no silence message for either device** across four reboots (salt 16:10 and 16:21, radar 16:24 and 16:59). The only Alert lines in the window belong to other sessions (AQM dropped-`/data` notice, sheet-health test lines). Caveat that applies to this method: the raw log only flushes on a state change / 15 min / another message, so the boot heartbeats themselves are not all in it -- they were captured directly over MQTT (times above); the *absence of an alert* is what this scan proves.
- **Found while verifying, tracked separately:** CARD-0340 (`switch.garage_presence_vswitch` never turns on; SmartThings entities in HA frozen since 2026-09-21). Also, unrelated to this card's firmware: `switch.salt_critical_alert`'s HA template helper has `turn_on` = `input_boolean.turn_on` **then** `input_boolean.turn_off` and no `turn_off` action, so it can never stay on (Salt Low Alert is built correctly) -- not yet carded or fixed.
- **Resolution, 2026-09-25 17:02 MST** (stamps corrected from 17:03/17:04 -- estimated forward instead of read; the carrying commit `ca0b638` is 17:02)**:** all five Done-when items met. (1) Both devices flashed, new firmware confirmed by reported hash (`0xde762c83`, `0xa946c0bf`). (2) After a deliberate restart of each -- salt by MQTT button, radar by power cycle (no button exists) -- the heartbeat arrived 84-90 s after boot with no watchdog alert, and the 30-minute cadence was seen to continue (16:51:20, 16:54:06). (3) Consumers unchanged: radar presence -> HA live-tested; salt -> HA written after boot. (4) Both YAMLs committed (`8f0c850`). (5) `watchdog-README.md` reconciled with §4.1.
- **Reflection (required last step):** the lasting lessons are in `JCTsh-Build-Standards.md` §4.1 (boot-time heartbeat rule and its applicability threshold) and in this card's method notes: compile with `python -m esphome` in `C:\esphome\<name>\`; verify by reported discovery `sw` hash, not by diffing generated source; an OTA reboot is a warm boot, so a power cycle is the stronger restart test where no button exists; verify an alert's *absence* from the Pi's log instead of asking for the dashboard password. **Side findings, carded separately:** CARD-0340 (vswitch never turns on / SmartThings entities frozen since 09-21) and CARD-0341 (`switch.salt_critical_alert` helper cannot stay on).
...
**Status:** Done -- RESOLVED 2026-09-26 10:56 MST
...
**Auto verify (RESOLVED 2026-09-26, see below):** -- the daily timer's first real scheduled firing (`config-drift-check.timer`, 09:00): `ssh pi@pi1.local "sudo journalctl -u config-drift-check.service --since '2026-09-24 08:55' --no-pager -o cat"` should show a `No drift.` run near 09:00. Everything else was verified live above by starting the installed service manually; only the timer itself firing unattended is unobserved. On a clean result, resolve per the marker protocol (edit this line to `**Auto verify (RESOLVED <date>, see below):**`) and move the card to Done.

**RESOLVED 2026-09-26 10:56 MST -- the marker above is satisfied; card closed.** Checked on the Pi: `config-drift-check.timer` last triggered `Sat 2026-09-26 09:00:01 MST` (one second after the scheduled time, i.e. the timer, not a hand-started run) and the service logged `No drift.`, `Deactivated successfully`, state file `{}`; next elapse `2026-09-27 09:00`. **One honest gap, so it isn't read as more than it is:** the specific 2026-09-24 09:00 and 09-25 09:00 runs can't be observed -- the Pi's journal only reaches back to `2026-09-26 04:50` (`journalctl --list-boots` shows a single boot entry even though `uptime -s` is 09-21), so those days aren't in it. A clean run writes no MQTT/log line, so there's no second record of them either. The marker's actual purpose -- an unattended scheduled firing working end to end -- *is* observed, on 09-26. Separate observation, not this card's: why that journal is that short despite `Storage=persistent` is worth a look by the ops cluster (CARD-0246 territory).
...
**Auto verify (RESOLVED 2026-09-26, see below):** -- the watchdog's first *scheduled* `Daily check` (cron `00 07 * * *`, fires at 07:00 with several hours of accumulated heartbeats): `ssh pi@pi1.local "sudo grep -h 'Daily check' /mnt/jctsh-logs/jctsh.log | tail -3"` should show a line like `Daily check: N components reporting, M silent (...)` with N > 0 and a ~07:00 timestamp. If it still says "no component heartbeats seen", the tracking function isn't receiving heartbeats and the fix is incomplete. On a good result, resolve per the marker protocol, tell the `tos` session about the self-test change if not yet done, and move the card to Done. (Note the durable log can lag up to ~15 min; `/status.json`'s `watchdog.last_seen` moving to ~07:00 is the fast signal.)

**RESOLVED 2026-09-26 10:56 MST -- the marker above is satisfied; card closed.** Three consecutive *scheduled* 07:00 runs are in the durable log, all with real counts (so the tracking function is receiving heartbeats and the counting path works, which the 09-23 manual trigger could not show): `2026-09-24 07:00` -> `5 components reporting, 2 silent (back-patio-temp-sensor, hiking-monitor)`; `09-25 07:00` -> `7 components reporting, 0 silent`; `09-26 07:00` -> `6 components reporting, 2 silent (air-quality-monitor, hiking-monitor)`. The silent lists match reality (hiking-monitor is only powered on hikes; the others were genuinely quiet), and the line is what `/status` now shows as the `watchdog` row's last message instead of a stale outage. **Loose end, not blocking:** the `tos` session still hasn't been told about the `kanban-pr-selftest.py` change (no `tos` session was live on 09-23 or now); the change is documented in that script's own docstring and this card, so it will be found on a read.
