...
- **Two gaps, stated rather than glossed:** (1) garage-radar was never restarted with the MQTT restart button -- its OTA reboot took the same boot path and produced the same ~90 s heartbeat, but it is not the literal deliberate restart Done-when 2 names. (2) "no watchdog alert" was not observed directly -- the dashboard's Alert scan needs `DASHBOARD_PASS`; `/status.json` showed both devices `Online` continuously through both reboots. Neither changes the firmware result; both are Joseph's call to accept or close.
