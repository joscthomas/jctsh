...
**Still not tested:** the field-simulation itself, still pending a clean run once the battery has had a real charge; a full bounded-attempt-window/15-min-retry cycle (needs a sustained no-network condition to observe, not yet arranged); and the `jctsh.duckdns.org` half of the broker switch specifically (only the `pi1.local`/home branch has been exercised live so far — the hotspot/field branch still needs a real test away from JCTnet1).
