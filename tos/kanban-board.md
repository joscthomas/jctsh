...
**Done when:** deployed to the Pi, and confirmed live against a real reboot (the next scheduled one, or a manual test) — the retained MQTT fact carries real before/after unavailable counts, `smartthings`/`ring` are confirmed auto-reloaded when applicable, and if `samsungtv` (or any other domain) appears on the slow-loading list, a real Alert fires naming it rather than silently reloading or silently doing nothing.
