...
**Same day, related and not yet fixed -- item 2 (QoS 1 on the replay path) is now demonstrably the bigger remaining problem:** the device logged `Replaying 121 hike readings...` and `Hike log replay complete.`, but only **23** of those 121 reached the Environmental Data Sheet (~19%); the 2026-09-19 hike showed the same shape (138 replayed, 19 in the Sheet). `hiking-monitor.yaml`'s `hike_log_replay_stream` still publishes each data line at **QoS 0** (`publish(..., line, 0, false)`, 50 ms apart) -- the identical pattern AQM's own replay had before this card's QoS 0->1 fix on it. Roughly 80% of every hike's environmental data is currently being silently dropped.
...
- **Back-patio offline / boot-then-vanish pattern** (above) — physical check of power (outlet live/GFCI, adapter temperature, LED) and, if the board is alive, a serial capture of a boot. Supersedes the RSSI-focused Watch for above as the working hypothesis; that marker stays as-is.
