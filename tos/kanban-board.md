...
**Not yet ordered** — design decided, part identified and verified real, sourcing link confirmed; purchase and build are still ahead. Stays in Defer per the 2026-08-19 decision (revisit at Planning when the enclosure is next opened) — this design work was done ahead of that trigger, at Joseph's initiative, but doesn't change the card's own sequencing.
...
- **Sensors: temp/humidity only** (BME280) for now — not the fuller LTR-390/SEN55 suite Phase 1 originally specified. Those stay documented above as a possible future expansion, not dropped, just not this build's target.
