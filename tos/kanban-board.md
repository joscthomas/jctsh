...
### CARD-0335 · [enhancement] [garage-radar] [salt-sensor] Retrofit the boot-time heartbeat (Build Standards §4.1) onto the two remaining 30-minute-heartbeat ESPHome devices — RESOLVED 2026-09-25 17:04 MST
**Status:** Done
...
- **Last gap closed, 2026-09-25 17:03 MST -- the watchdog-alert check, done from the Pi's durable log instead of the dashboard (no `DASHBOARD_PASS` needed; same source the dashboard reads).** `awk` over `/mnt/jctsh-logs/jctsh.log` from 15:50 to 17:02 for Alert-category lines, `watchdog` as component, and `silent`/`not reported` text: **no watchdog line and no silence message for either device** across four reboots (salt 16:10 and 16:21, radar 16:24 and 16:59). The only Alert lines in the window belong to other sessions (AQM dropped-`/data` notice, sheet-health test lines). Caveat that applies to this method: the raw log only flushes on a state change / 15 min / another message, so the boot heartbeats themselves are not all in it -- they were captured directly over MQTT (times above); the *absence of an alert* is what this scan proves.
