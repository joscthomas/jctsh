...
**Done when:** today's real 2026-09-08 session (2h10m, out to ~800m and back, ~78min stationary rest in the middle) reclassifies as `is_hike: true` with the rest period correctly reflected in `stopped_time_min`, not `moving_time_min`; a synthetic or real all-stationary/GPS-drift-while-parked session (CARD-0100's precedent case) still correctly rejects; and a synthetic all-vehicle-speed session still correctly rejects on the existing "too fast" branch (untouched by this fix).
