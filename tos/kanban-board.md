...
4. **Still open, Joseph's call:** given the above, is a ~0.4% lone-lookup failure rate worth changing CARD-0279's retry/alert behavior for (e.g. suppressing the Alert for a stationary device's isolated miss), or is 3 Alerts in 66 hours an acceptable, expected noise level not worth building anything against?

**Done when:** not yet scoped — pending Joseph's call on open question 4 above.
