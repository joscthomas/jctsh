...
- **One gap remains, stated rather than glossed:** (1) ~~garage-radar was never restarted deliberately~~ -- closed by the power cycle above. (2) "no watchdog alert" was not observed directly -- the dashboard's Alert scan needs `DASHBOARD_PASS`; `/status.json` showed both devices `Online` continuously through both reboots. Neither changes the firmware result; both are Joseph's call to accept or close.
...
- **Still open:** firmware YAML is committed (`8f0c850`); the remaining question is whether to close on the two gaps above.
