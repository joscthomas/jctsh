...
- **Dock-detect raw check: verified, 2026-09-08.** Added a `binary_sensor` for GPIO32 (pin 7) to `minimal-test.yaml` (wasn't present before), reflashed. Its `on_state`→`logger.log` never actually produced UART output (same pre-existing, already-known gap as the Intent switch's own logging — not a new problem, deferred rather than chased), so verified directly with a multimeter instead: GPIO32 (pin 7) read ~0V with USB unplugged from TP4056 (LOW/field), and a real positive voltage with USB plugged in (HIGH/docked) — matches the 68kΩ/100kΩ divider design exactly.
