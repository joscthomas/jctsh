...
**Auto verify marker resolved 2026-09-14 (was: `2026-09-14 03:15 MST`) — checked live, real gap found.** After today's actual scheduled reboot (boot at 03:00:35), the persistent snapshot's last pre-reboot entries are `Starting scheduled-reboot.service` / `Starting journal-snapshot.service` at 03:00:01 — then the log jumps straight to the next boot's `kernel: Booting Linux...` line at 03:00:35. The 34-second gap between those two lines (where systemd's actual `Stopping <unit>...`/`Reached target Shutdown` sequence would run) is **not captured** — `sudo journalctl -u journal-snapshot-onshutdown.service` also shows no entries (journald itself is volatile across the reboot, so that's expected and uninformative either way). This means the fix reaches the point where shutdown is *triggered*, but not the shutdown sequence itself — likely an inherent timing limit of `Before=shutdown.target`: the hook fires at the *start* of the shutdown sequence, before systemd has generated the "stopping various units" log lines it's meant to catch, not after. **No marker re-armed here** — this isn't something a future reboot will resolve passively; the gap is a design/ordering question that needs an actual fix attempt before another live check would tell us anything new.

**Root-caused and fixed live, 2026-09-14, across 5 real Pi reboots — two distinct bugs found, not one.** Once the ordering gap above was noticed, isolating and fixing it required watching the actual shutdown sequence directly rather than guessing from the systemd manual, since journald's own volatile storage (CARD-0246) destroys any evidence of *why* a job silently fails to run — there's no error to read after the fact, only the absence of expected output.

**Bug 1 (the ordering gap already described above) — fixed via explicit `After=`.** `Before=shutdown.target` alone only guarantees the unit finishes *sometime* before `shutdown.target` completes; it races in parallel with every other service's stop job, with no guarantee of running *after* them. Added `After=docker.service mosquitto.service nodered.service` — the three services whose shutdown-time log lines actually motivated this card in the first place (Docker's container stop sequence, Mosquitto's connection-loss lines, Node-RED). Considered and rejected `final.target` (systemd.special(7): "pull in late services after all normal services are already terminated") — it also runs after all mounts are unmounted, which would break the write to `/mnt/jctsh-logs`.

**Bug 2, found while verifying Bug 1's fix — much bigger than the ordering gap: the unit had never once actually fired during any real reboot, since the original 2026-09-06 build.** Confirmed by grepping the full snapshot-log history for `journal-snapshot-onshutdown` — every occurrence, going back to build time, was a manual `systemctl start`, never a real shutdown transaction, despite `systemctl show reboot.target -p Wants` correctly listing it. Isolated via controlled live testing (a minimal test unit with the same `DefaultDependencies=no`/`Before=shutdown.target`/`WantedBy=` shape, no `RequiresMountsFor`, fired correctly across 3 consecutive real reboots; an otherwise-identical unit with `RequiresMountsFor=/mnt/jctsh-logs` added — present in the original build from day one — never fired, in 2 consecutive reboots). **Root cause:** `RequiresMountsFor=` adds a *hard* `Requires=` on the mount unit, not just ordering. During real shutdown, that mount unit is itself a normal default-dependencies unit slated to be torn down before `shutdown.target` completes, same as everything else — a unit that both demands the mount stay active (`Requires=`) and is itself ordered `Before=shutdown.target` creates a genuine ordering conflict, which systemd resolves by silently dropping the job. No visible error anywhere, since whatever systemd logged about that decision is gone with the volatile journal before it can be read. **Fix:** dropped `RequiresMountsFor` entirely, relying instead on the empirically-proven timing (`DefaultDependencies=no` + `Before=shutdown.target` always runs well before local-fs unmounting, which only happens after `shutdown.target` completes via `final.target`).

**Both fixes combined and verified live, 2026-09-14 13:24-13:25 MST — the real, final confirmation reboot.** `journal-snapshot.log` shows `mosquitto.service`/`nodered.service` stopped at 13:24:27, `docker.service` (with its container force-shutdown messages) at 13:24:38, and `Starting journal-snapshot-onshutdown.service...` at 13:24:38 — immediately after all three, in the same second `docker.service` finished. This is exactly the diagnostic picture CARD-0248 was originally raised to capture (the class of connectivity-failure messages from the 2026-09-06 incident — Nabu Casa tunnel error, SmartThings API timeout, Ring push exception, MQTT connection lost — would all land in this exact window). All temporary diagnostic units (`shutdown-diag-test`, `shutdown-diag-mount`, `shutdown-diag-stream`) and their log files removed from the Pi afterward — nothing left behind from the investigation.

**Done when:** deployed and enabled on the Pi, and confirmed live against a real reboot capturing the intended shutdown-sequence content — **met**, both conditions verified together in the 2026-09-14 13:24 MST reboot above.
