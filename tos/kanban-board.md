...
**Build progress 2026-09-21 18:46 MST — wiring docs sharpened and the board's silkscreen verified against the pin table.** `wiring.md` now names every ESP32 pin by both silkscreen label and physical pin number (`GPIO21, pin 33`, `GPIO22, pin 36`, `3V3, pin 1`, `GND, pin 38`) and carries a new board-side ESP32 Connections table alongside the two sensor-side ones. Joseph photographed the actual board (`esp32-pins-photo.jpg`) and all 38 silkscreen labels were compared against `ESP32-project-pins.md`: **37 match exactly**, including all four pins this build uses.
