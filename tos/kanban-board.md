...
### CARD-0204 · [enhancement] [hike-izer] Environmental Data (temp/humidity/pressure/UV) on the Elevation & Speed chart
**Status:** Build
...
**Done when:** ~~the new panel renders on a real hike page with real Environmental Data~~ — met, verified above against real 2026-08-22 data in a scratch render. ~~the legend toggle correctly switches between both pairings~~ — built, structurally confirmed (both mode groups + both toggle buttons present); not yet clicked in a live browser. ~~a real gap in sensor data is confirmed to interpolate~~ — met via the standalone unit test against synthetic gap data (the real 2026-08-22 hike happened to have continuous coverage, so it didn't exercise a real gap). ~~hovering the new panel correctly syncs the Route Map and existing chart's highlight~~ — built (shared event names, shared `data-index`), not yet confirmed via a real live-browser hover. ~~both templates render it identically~~ — met for `templating.py` (live-tested); `html-template.html` has the matching placeholder/CSS but has no code path of its own to test outside a real interactive Skill run. **Not yet done:** an actual live-browser click-through (toggle, hover-sync, expand) — recommend a quick manual check next time the hike-izer site is open, same "structurally verified, not yet eyeballed" pattern several other hike-izer cards this project have shipped under and closed out later once actually looked at.

**Related:** CARD-0110 (the Elevation & Speed chart this extends), CARD-0082 (Route Map, the hover-sync partner), CARD-0133 (`interpolate_position()`, the interpolation-with-cap precedent this reuses), CARD-0194 item 4 (the chart-expand feature this reused directly), CARD-0197 (GPS-correlation-timing instrumentation — checked, no blocking interaction found), `components/hike-izer/build_hike_chart.py`, `components/hike-izer/fetch_hike_data.py`, `components/hike-izer-orchestrator/templating.py`, `components/hike-izer/html-template.html`, `.claude/skills/hike-izer/SKILL.md`.
...
**Flashed 2026-08-24 (OTA)** as part of the broader hiking-monitor firmware update that also carried CARD-0195/CARD-0198/CARD-0200. Ran on the real 2026-08-24 hike ("Boulder Pass Loop") — Joseph reported not consciously noticing the refresh frequency, which is consistent with the design (only ~4 refreshes total over a ~76-min field session, ~20 min apart) but wasn't independently confirmable from any existing log data, since e-ink `update()` calls weren't logged anywhere.
...
**Done when:** ~~the display throttle is flashed~~ — met, 2026-08-24. ~~the Node-RED routing works~~ — met, verified live with real synthetic MQTT events (above). **Still open:** a future real hike's actual `display_refresh` log entries checked against real wall-clock spacing to confirm the ~20min cadence is genuinely holding — the mechanism is proven end-to-end, just not yet exercised by a real hike. The solar-habit note has no completion criteria — it's not a deliverable.

**Related:** CARD-0201 (broken-out sleep rearchitecture), CARD-0203 (broken-out LiPo fit check), CARD-0070 (deferred boost-converter/LDO swap), CARD-0195 (sibling diagnostic-instrumentation card from the same investigation), `components/hiking-monitor/hiking-monitor.yaml`, `components/hiking-monitor/power-system.md`.
...
**Still open: the ESP32 firmware side.** `esphome config` validates clean against the synced `C:\esphome\hiking-monitor\` copy, but the device itself hasn't been flashed — so the Node-RED routing is live and correctly wired, but nothing has actually exercised it with a real `skip`/`reset` record yet. Full end-to-end verification (a real skip or reset event flowing correctly to `/log` instead of the Sheet) still needs the firmware flashed first — no physical device access from this session.
...
### CARD-0194 · [idea] [hike-izer] Iterative hike-izer webpage improvements from the 2026-08-22 hike
**Status:** Build
...
**2. Manual observation-text editing on the published page — built and deployed, not yet live-verified.** Joseph: "I want to be able to manually fix text on the web page." Interviewed and scoped: **observations only** (not narrative or other sections); auth is a **simple shared PIN**, entered via **a secret method** rather than a visible button (Joseph's phrasing) — implemented as a hidden trigger: typing the keyword **"unlock"** anywhere on the page reveals the edit UI. Write-back mechanism: **option 3** — a per-hike JSON overrides file, not a direct Sheet edit and not narrative regeneration.
   ...
   **Not yet done:** live end-to-end verification against a real published hike page (unlock → PIN → edit → save → confirm both the overrides file and the live HTML updated); nothing from this sub-item committed to git yet.

**3. Per-section data-source attribution — built, not yet deployed.** Joseph wanted each data section to note where its data actually comes from, rather than leaving readers to guess. Confirmed the real source of each section (some assumed names weren't quite right) before labeling:
