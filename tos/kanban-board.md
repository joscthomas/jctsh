...
4. **Re-run the minimal-test staged sequence against the new hardware**, 4 consecutive clean cycles per stage before advancing (matching this card's own 2026-08-24 base-platform bar), pausing to diagnose at any regression.
   ...
   - Stage 2 (+MQTT), Stage 3 (+I2C/SEN55) — not yet reached. Both run under the same revised power protocol above (USB in TP4056's port throughout, Power Connected true) — not a separate reminder, the same requirement. Stage 3 should specifically try to provoke the coincident-load scenario the redesign targets (trigger WiFi association while SEN55 is actively in Measurement mode), not just let both run and hope they overlap.
